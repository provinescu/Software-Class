...
";
		...
		MainMenu.register(~menuFile.title_("File for Analyze"), "WekAgentsTools");
		...
		MainMenu.register(~menuFX, "WekAgentsTools");
		...
		MainMenu.register(~menuRecording.title_("Recording"), "WekAgentsTools");
		...
		MainMenu.register(~menuAudio.title_("Audio"), "WekAgentsTools");
		...
		MainMenu.register(~menuAlgo.title_("Algorithm"), "WekAgentsTools");
		...
		MainMenu.register(~menuMidi.title_("MIDI"), "WekAgentsTools");
		...
		MainMenu.register(menuWek.title_("Wekinator"), "WekAgentsTools");

		~menuKeys = Menu(
			MenuAction("List ShortCuts WekAgents", {
				//Document.new("ShortCut for WekAgents", ~helpHPWekAgents);
				TextView().name_("ShortCut for WekAgents").string_(~helpHPWekAgents).front
			};
			),
		...
		MainMenu.register(~menuKeys.title_("ShortCuts"), "WekAgentsTools");
		...
		~fonctionRecOn={
			...
			if(~flagRecording == 'off', {
				...
				s.bind{
					...
					s.sync;
					s.prepareForRecord("~/Music/SuperCollider Recordings/".standardizePath ++ "WekAgents_" ++ Date.localtime.stamp ++ "." ++ ~headerFormat);
					s.sync;
					s.record;
					s.sync;
				};
	...
	run {
		...
		~audioFile=Synth.newPaused("OSC WekAgents File Onsets",
			['busFileIn', ~busFileIn.index], ~groupeAnalyse, \addToTail);
		s.sync;
		~audioIn=Synth.newPaused("OSC WekAgents Onsets",
			[\seuil, 0.5 ], ~groupeAnalyse, \addToTail);
		s.sync;
		// Creation Synth Tempo
		~tempoFile=Synth.newPaused("OSC WekAgents Tempo FileIn",
			['busFileIn', ~busFileIn.index], ~groupeAnalyse, \addToTail);
		s.sync;
		~tempoIn=Synth.newPaused("OSC WekAgents Tempo AudioIn",
		...
		~creationSynthDefAnalyze={arg algo;
			s.bind{
				...
				s.sync;
				switch(algo,
					0, {~audioFile=Synth.newPaused("OSC WekAgents File Onsets",
					...
					1, {~audioFile=Synth.newPaused("OSC WekAgents File Pitch",
					...
					s.sync;
					~audioIn=Synth.newPaused("OSC WekAgents Pitch",
						[\seuil, ~seuilInSlider.value, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1)], ~groupeAnalyse, \addToTail);
					s.sync;
					~tempoFile=Synth.newPaused("OSC WekAgents Tempo FileIn",
						['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
					...
					2, {~audioFile=Synth.newPaused("OSC WekAgents File Pitch2",
						[\bufferplay, ~bufferanalysefile.bufnum, 'busFileIn', ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilInSlider.value], ~groupeAnalyse, \addToTail);
					s.sync;
					~audioIn=Synth.newPaused("OSC WekAgents Pitch2",
						[\seuil, ~seuilInSlider.value, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1)], ~groupeAnalyse, \addToTail);
					s.sync;
					~tempoFile=Synth.newPaused("OSC WekAgents Tempo FileIn",
						['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
					...
					3, {~audioFile=Synth.newPaused("OSC WekAgents File KeyTrack",
						[\bufferplay, ~bufferanalysefile.bufnum, 'busFileIn', ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilInSlider.value], ~groupeAnalyse, \addToTail);
					s.sync;
					~audioIn=Synth.newPaused("OSC WekAgents KeyTrack",
						[\seuil, ~seuilInSlider.value, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1)], ~groupeAnalyse, \addToTail);
					s.sync;
					~tempoFile=Synth.newPaused("OSC WekAgents Tempo FileIn",
						['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
					...
					4, {~audioFile=Synth.newPaused("OSC WekAgents File Keyboard",
						['busFileIn', ~busFileIn.index, \trigger, 0], ~groupeAnalyse, \addToTail);
					s.sync;
					~audioIn=Synth.newPaused("OSC WekAgents Keyboard",
		...
		~synthPlayFile=Synth.newPaused("WekAgents Play File",
		...
		})}, '/WekAgents_Analyse_Tempo', ~serverAdresse);
		...
		OSCFunc.newMatching({arg msg, time, addr, recvPort;
			...
			{
				~synthDefInstrMenu.lo = wekOut[0];//0
				~synthDefInstrMenu.hi = wekOut[1];
				~soundsInstrMenu.lo = wekOut[2];
				~soundsInstrMenu.hi = wekOut[3];
				~freqInstr.lo = wekOut[4];
				~freqInstr.hi = wekOut[5];
				~freqTransposeInstr.value = wekOut[6];
				~ampInstr.lo = wekOut[7];
				~ampInstr.hi = wekOut[8];
				~dureeInstr.lo = wekOut[9];
				~dureeInstr.hi = wekOut[10];
				~dureeTempoinstr.value = wekOut[11];
				~quantaMusicSlider.value = wekOut[12];
				// Genes
				~geneFreqRanger.lo = wekOut[13];// 13
				~geneFreqRanger.hi = wekOut[14];
				~geneFreqButton.value = wekOut[15];
				~geneTransFreqRanger.lo = wekOut[16];
				~geneTransFreqRanger.hi = wekOut[17];
				~geneTransFreqButton.value = wekOut[18];
				~geneAmpRanger.lo = wekOut[19];
				~geneAmpRanger.hi = wekOut[20];
				~geneAmpButton.value = wekOut[21];
				~geneDureeRanger.lo = wekOut[22];
				~geneDureeRanger.hi = wekOut[23];
				~geneDureeButton.value = wekOut[24];
				~geneMulDureeRanger.lo = wekOut[25];
				~geneMulDureeRanger.hi = wekOut[26];
				~geneMulDureeButton.value = wekOut[27];
				~genePanRanger.lo = wekOut[28];
				~genePanRanger.hi = wekOut[29];
				~genePanButton.value = wekOut[30];
				~geneBufferRanger.lo = wekOut[31];
				~geneBufferRanger.hi = wekOut[32];
				~geneBufferButton.value = wekOut[33];
				~geneSampleRangerLow.value = wekOut[34];// 34
				~geneSampleRangerHigh.value = wekOut[35];// 35
				~geneSampleButton.value = wekOut[36];// 36
				~geneReverseRanger.lo = wekOut[37];
				~geneReverseRanger.hi = wekOut[38];
				~geneReverseButton.value = wekOut[39];
				~geneLoopRanger.lo = wekOut[40];
				~geneLoopRanger.hi = wekOut[41];
				~geneLoopButton.value = wekOut[42];
				~geneOffsetRanger.lo = wekOut[43];
				~geneOffsetRanger.hi = wekOut[44];
				~geneOffsetButton.value = wekOut[45];
				~geneEnvLevelRanger.lo = wekOut[46];
				~geneEnvLevelRanger.hi = wekOut[47];
				~geneEnvLevelButton.value = wekOut[48];
				~geneEnvDureeRanger.lo = wekOut[49];
				~geneEnvDureeRanger.hi = wekOut[50];
				~geneEnvDureeButton.value = wekOut[51];
				~geneSynthRangerLow.value = wekOut[52];// 52
				~geneSynthRangerHigh.value = wekOut[53];// 53
				~geneSynthButton.value = wekOut[54];// 54
				~geneControlsRanger.lo = wekOut[55];
				~geneControlsRanger.hi = wekOut[56];
				~geneControlsButton.value = wekOut[57];
				~geneAudioOutRanger.lo = wekOut[58];
				~geneAudioOutRanger.hi = wekOut[59];
				~geneAudioOutButton.value = wekOut[60];
				~geneMidiOutRanger.lo = wekOut[61];
				~geneMidiOutRanger.hi = wekOut[62];
				~geneMidiOutButton.value = wekOut[63];
				~geneInputRangerLow.value = wekOut[64];// 64
				~geneInputRangerHigh.value = wekOut[65];// 65
				~geneInputButton.value = wekOut[66];// 66
				~geneLoopMusicRanger.lo = wekOut[67];
				~geneLoopMusicRanger.hi = wekOut[68];
				~geneLoopMusicButton.value = wekOut[69];
				~geneBufferMusicRanger.lo = wekOut[70];
				~geneBufferMusicRanger.hi = wekOut[71];
				~geneBufferMusicButton.value = wekOut[72];
				~geneChordMaxRanger.lo = wekOut[73];
				~geneChordMaxRanger.hi = wekOut[74];
				~geneChordMaxButton.value = wekOut[75];
				~geneChordDurRanger.lo = wekOut[76];
				~geneChordDurRanger.hi = wekOut[77];
				~geneChordDurButton.value = wekOut[78];
				~geneAlgorithm.lo = wekOut[79];
				~geneAlgorithm.hi = wekOut[80];
				~geneAlgorithmButton.value = wekOut[81];
				~geneBand0.value = wekOut[82];// 82
				~geneBand1.value = wekOut[83];
				~geneBand2.value = wekOut[84];
				~geneBand3.value = wekOut[85];
				~geneBand4.value = wekOut[86];
				~geneBand5.value = wekOut[87];
				~geneBand6.value = wekOut[88];
				~geneBand7.value = wekOut[89];
				~geneBand8.value = wekOut[90];
				~geneBand9.value = wekOut[91];
				~geneBand10.value = wekOut[92];
				~geneBand11.value = wekOut[93];
				~geneBand12.value = wekOut[94];
				~geneBandButton.value = wekOut[95];
			}.defer(0);

			// Preset
			numPreset = wekOut[96];

			if(numPreset != lastNumPreset and: {(time - lastTimeWek) >= 1},
				// load new preset
				{
					{
			...
			if(~entreemode == 'Audio' or: {~entreemode == 'File'},
				{
					...
					if(flagStreamMFCC != 'wek',
						{
							// Music Controls
							data = data.add(~synthDefInstrMenu.lo);// 0
							data = data.add(~synthDefInstrMenu.hi);
							data = data.add(~soundsInstrMenu.lo);
							data = data.add(~soundsInstrMenu.hi);
							...
							// Genes
							data = data.add(~geneFreqRanger.lo);// 13
							data = data.add(~geneFreqRanger.hi);
							data = data.add(~geneFreqButton.value);
							...
							data = data.add(~geneSampleRangerLow.value);// 34
							data = data.add(~geneSampleRangerHigh.value);// 35
							data = data.add(~geneSampleButton.value);// 36
							...
							data = data.add(~geneSynthRangerLow.value);// 52
							data = data.add(~geneSynthRangerHigh.value);// 53
							data = data.add(~geneSynthButton.value);// 54
							...
							data = data.add(~geneInputRangerLow.value);// 64
							data = data.add(~geneInputRangerHigh.value);// 65
							data = data.add(~geneInputButton.value);// 66
							...
							data = data.add(~geneBand0.value);// 82
		...
		}, '/WekAgents_Analyse_Audio', ~serverAdresse);

		// Analyse Midi-IN
		~fonctionOSCMidiIn={
			~oscMidiIn = MIDIdef.noteOn(\midiNoteOn, {arg amp, freq, canal, src;
				var duree=0, time=Main.elapsedTime, mfcc, data;
				duree = time - ~lastTimeAnalyse;
				// Normalise
				freq=freq/127;
				amp=amp/127;
	...
	genesPanel {
		...
		// Music
		StaticText(~wg, Rect(0, 0, 300, 15)).string_("Musical Genome").stringColor_(Color.magenta).font_(Font("Georgia", 12));
		~wg.view.decorator.nextLine;
		~geneFreqRanger=EZRanger(~wg, 300 @ 18, "Frequency", ControlSpec(0, 127, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneFreqRanger", view.value)})}, [24, 108], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneFreqButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneTransFreqRanger=EZRanger(~wg, 300 @ 18, "Translate Frequency", ControlSpec(-127, 127, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneTransFreqRanger", view.value)})}, [-12, 12], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneTransFreqButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneAmpRanger=EZRanger(~wg, 300 @ 18, "Amplitude", ControlSpec(-inf, 0, \db, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneAmpRanger", view.value)})}, [-inf, 0], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneAmpButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneDureeRanger=EZRanger(~wg, 300 @ 18, "Duration", ControlSpec(0, 60, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneDureeRanger", view.value)})}, [0, 4], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneDureeButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneMulDureeRanger=EZRanger(~wg, 300 @ 18, "Stretch Duration", ControlSpec(-16, 64, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneMulDureeRanger", view.value)})}, [-2, 4], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneMulDureeButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~genePanRanger=EZRanger(~wg, 300 @ 18, "Pan", ControlSpec(-1, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~genePanRanger", view.value)})}, [-1, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~genePanButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneSampleRangerLow=PopUpMenu(~wg,Rect(0,0, 148, 18)).items_(~displaySons).background = Color.magenta;
		...
		~geneSampleRangerHigh=PopUpMenu(~wg,Rect(0,0, 148, 18)).items_(~displaySons).background = Color.magenta;
		...
		~geneSampleButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneReverseRanger=EZRanger(~wg, 300 @ 18, "Reverse Sample", ControlSpec(-1, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneReverseRanger", view.value)})}, [-1, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneReverseButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneLoopRanger=EZRanger(~wg, 300 @ 18, "Loop Sample", ControlSpec(-1, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneLoopRanger", view.value)})}, [-1, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneLoopButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneOffsetRanger=EZRanger(~wg, 300 @ 18, "Offset Sample", ControlSpec(0, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneOffsetRanger", view.value)})}, [0, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneOffsetButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneEnvLevelRanger=EZRanger(~wg, 300 @ 18, "Envelope Level", ControlSpec(0, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneEnvLevelRanger", view.value)})}, [0.1, 0.9], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneEnvLevelButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneEnvDureeRanger=EZRanger(~wg, 300 @ 18, "Envelope Duration", ControlSpec(0, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneEnvDureeRanger", view.value)})}, [0.125, 0.75], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneEnvDureeButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneSynthRangerLow=PopUpMenu(~wg,Rect(0,0, 148, 18)).items_(~listSynth).background = Color.magenta;
		...
		~geneSynthRangerHigh=PopUpMenu(~wg,Rect(0,0, 148, 18)).items_(~listSynth).background = Color.magenta;
		...
		~geneSynthButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)], ["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneControlsRanger=EZRanger(~wg, 300 @ 18, "Control Synthesizer", ControlSpec(0, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneControlsRanger", view.value)})}, [0, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneControlsButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneAudioOutRanger=EZRanger(~wg, 300 @ 18, "Audio Out", ControlSpec(1, ~sourceOutAgents.size, \lin, 1), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneAudioOutRanger", view.value)})}, [1, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneAudioOutButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneMidiOutRanger=EZRanger(~wg, 300 @ 18, "Midi Out", ControlSpec(0, 16, \lin, 1), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneMidiOutRanger", view.value)})}, [1, 16], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneMidiOutButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		StaticText(~wg, Rect(0, 0, 52, 15)).string_("Audio In").stringColor_(Color.magenta);
		~geneInputRangerLow=PopUpMenu(~wg,Rect(0,0, 120, 18)).items_(~audioInLR).background = Color.magenta;
		...
		~geneInputRangerHigh=PopUpMenu(~wg,Rect(0,0, 120, 18)).items_(~audioInLR).background = Color.magenta;
		...
		~geneInputButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneLoopMusicRanger=EZRanger(~wg, 300 @ 18, "Loop Buffer Music", ControlSpec(-1, 1, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneLoopMusicRanger", view.value)})}, [-1, 1], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneLoopMusicButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneBufferMusicRanger=EZRanger(~wg, 300 @ 18, "Buffer Music", ControlSpec(1, 256, \exp, 1), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneBufferMusicRanger", view.value)})}, [6, 24], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneBufferMusicButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneChordMaxRanger=EZRanger(~wg, 300 @ 18, "Chord Max", ControlSpec(1, 12, \exp, 1), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneChordMaxRanger", view.value)})}, [3, 6], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneChordMaxButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		~wg.view.decorator.nextLine;
		~geneChordDurRanger=EZRanger(~wg, 300 @ 18, "Chord Dur", ControlSpec(0.01, 1, \exp, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneChordDurRanger", view.value)})}, [0.0625, 0.0833], false, 125, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneChordDurButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		// Algo Choice
		StaticText(~wg, Rect(0, 0, 46, 15)).string_("Algorithm").stringColor_(Color.magenta);
		~geneAlgorithm=EZRanger(~wg, 250 @ 18, "Algorithm", ControlSpec(0, ~listeAlgorithm.size - 1, \lin, 1), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneAlgorithm", view.value)})}, [0, ~listeAlgorithm.size - 1], false, 70, 40).setColors(Color.grey(0.3), Color.magenta);
		~geneAlgorithmButton=Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		...
		states_([["1", Color.magenta, Color.green(0.8, 0.25)], ["1", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["2", Color.magenta, Color.green(0.8, 0.25)], ["2", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["3", Color.magenta, Color.green(0.8, 0.25)], ["3", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["4", Color.magenta, Color.green(0.8, 0.25)], ["4", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["5", Color.magenta, Color.green(0.8, 0.25)], ["5", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["6", Color.magenta, Color.green(0.8, 0.25)], ["6", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["7", Color.magenta, Color.green(0.8, 0.25)], ["7", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["8", Color.magenta, Color.green(0.8, 0.25)], ["8", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["9", Color.magenta, Color.green(0.8, 0.25)], ["9", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["10", Color.magenta, Color.green(0.8, 0.25)], ["10", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["11", Color.magenta, Color.green(0.8, 0.25)], ["11", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		...
		states_([["12", Color.magenta, Color.green(0.8, 0.25)], ["12", Color.magenta, Color.red(0.8, 0.25)]]).font_(Font("Helvetica", 10)).
		action_({arg band; ~flagBandGenes.put(12, band.value);
			if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~geneBand12", band.value)});
		});

		~geneBandButton = Button(~wg,Rect(0, 0, 25, 18)).states=[["On", Color.magenta, Color.green(0.8, 0.25)],["Off", Color.magenta, Color.red(0.8, 0.25)]];
		~geneBandButton.action = {arg flag;
	...
	synthPanel {
		...
		~synthDefInstrMenu.background = Color.magenta;
		...
		~soundsInstrMenu.background = Color.magenta;
		...
		~wi.view.decorator.nextLine;
		// FREQ
		~freqInstr=EZRanger(~wi, 200 @ 18, "Fhz", ControlSpec(0, 127, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~freqInstr", view.value)})}, [0, 127], false, 25, 30).setColors(Color.grey(0.3), Color.magenta);
		// Translate
		~freqTransposeInstr=EZKnob(~wi, 125 @ 18, "Translate", ControlSpec(-127, 127, \lin, 0), {arg tran; tran.value; if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~freqTransposeInstr", tran.value)})}, 0, labelWidth: 60,unitWidth: 0, layout: 'horz').setColors(Color.grey(0.3), Color.magenta);
		~wi.view.decorator.nextLine;
		// AMP
		~ampInstr=EZRanger(~wi, 200 @18, "Amp", \db,
			{|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~ampInstr", view.value)})}, [-inf, 0], false, 25, 30).setColors(Color.grey(0.3), Color.magenta);
		~wi.view.decorator.nextLine;
		// DUREE
		~dureeInstr=EZRanger(~wi, 200 @ 18, "Dur", ControlSpec(0, 60, \lin, 0), {|view| if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~dureeInstr", view.value)})}, [0, 4], false, 25, 30).setColors(Color.grey(0.3), Color.magenta);
		// Stretch
		~dureeTempoinstr=EZKnob(~wi, 125 @ 18, "Stretch", ControlSpec(-16, 64, \lin, 0), {arg tempo; tempo.value; if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~dureeTempoinstr", tempo.value)})}, 1, labelWidth: 55,unitWidth: 0, layout: 'horz').setColors(Color.grey(0.3), Color.magenta);
		// Quant
		~quantaMusicSlider=EZKnob(~wi, 125 @ 18, "Quant",ControlSpec(1, 100, \exp, 0),
			{|ez| ~tempoMusicPlay.schedAbs(~tempoMusicPlay.beats, {~quantaMusic=ez.value;nil});
				if(~flagScoreRecordGUI == 'on', {~fonctionRecordScore.value("~quantaMusicSlider", ez.value)})},~quantaMusic, labelWidth: 35,unitWidth: 0, layout: 'horz').setColors(Color.grey(0.3), Color.magenta);
	...
	controlPanel {
		...
		StaticText(~wp, Rect(0, 0, 500, 20)).string_("WekAgents by Provinescu's Software Production").stringColor_(Color.white).font_(Font("Georgia", 12)).align = \right;
		~wp.view.decorator.nextLine;
		StaticText(~wp, Rect(10,10,500,20)).string_("System WekAgents").stringColor_(Color.white).font_(Font("Georgia-BoldItalic", 12));
		~wp.view.decorator.nextLine;
		...
		~viewpopulation = StaticText(~wp, Rect(10,10, 65, 20)).string_("Crew"+~agents.asString).stringColor_(Color.yellow);
		~viewagesagents = StaticText(~wp, Rect(10,10, 65, 20)).string_("Age"+~ages.mediane.asString).stringColor_(Color.yellow);
		~viewviesagents = StaticText(~wp, Rect(10,10, 80, 20)).string_("Life"+~vies.mediane.asString).stringColor_(Color.yellow);
		~viewenfantsagents = StaticText(~wp, Rect(10,10, 80, 20)).string_("Children"+~enfants.mediane.asString).stringColor_(Color.yellow);
		~viewvoisins = StaticText(~wp, Rect(10,10, 100, 20)).string_("Neightboring").stringColor_(Color.yellow);
		...
		// Wekinator
		Button(~wp, Rect(0, 0, 70, 20)).states_([["WekRec On", Color.magenta], ["WekRec Off", Color.red]]).action_({|view|
			switch(view.value,
	...
	shortCuts {
		...
		~foncSaveUnivers={arg datafile, flagGenome='on', flagSequence='on';//herite de foncSave monde
			...
			// Gene
			datafile=datafile.add(~geneBandButton.value);//174
		...
		~loadUnivers={arg file, flagGenome='on', flagSequence='on';
			...
			// Gene
			~geneBandButton.valueAction = datafile.wrapAt(174);
		...
		if(flag == true, {
			...
			SynthDef("OSC WekAgents Onsets",
				{arg in=0,  seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
					var input, detect, freqin, hasfreqin, ampin, centroid, flatness, fft, energy, flux, inputFilter, array;
					input= SoundIn.ar(in);
					...
					SendReply.kr(detect, '/WekAgents_Analyse_Audio', values: [freqin, ampin, centroid, energy, flux.clip(0.0001, 1), flatness.clip(0.0001, 1)] ++ array, replyID: 1);
			}).send(s);

			// New Analyse Audio  2
			SynthDef("OSC WekAgents Pitch",
			...
			// New Analyse Audio  4
			SynthDef("OSC WekAgents KeyTrack",
				{arg in=0, seuil=0.5, filtre=1;
					var input, detect, freqin, ampin, centroid, flatness, fft, energy, key, flux, array;
					input= SoundIn.ar(in);
			...
			// Keyboard
			SynthDef("OSC WekAgents Keyboard",
				{arg in=0, note=60, amp=0.5, trigger = 0;
					var input, centroid, flatness, fft, energy, key, flux, array;
					input= SoundIn.ar(in);
					fft = FFT(LocalBuf(1024, 1), input);
					array = MFCC.kr(fft);// 13 a 40 Bands
					centroid = SpecCentroid.kr(fft);
					...
					SendReply.kr(trigger, '/WekAgents_Analyse_Audio', values: [note, amp, centroid, energy, flux.clip(0.0001, 1), flatness.clip(0.0001, 1)] ++ array, replyID: 1);
			}).add;

			// New Analyse File
			SynthDef("OSC WekAgents File Onsets",
			...
			SynthDef("OSC WekAgents File Pitch",
			...
			SynthDef("OSC WekAgents File Pitch2",
				{arg  busFileIn, seuil=0.5, filtre=0, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
			...
			SynthDef("OSC WekAgents File KeyTrack",
				{arg  busFileIn, seuil=0.5, filtre=1;
					var detect, freqin, ampin, input, centroid, flatness, fft, energy, key, flux, array;
					input = In.ar(busFileIn);
					detect= Onsets.kr(FFT(LocalBuf(512, 1), input), seuil);
					key = KeyTrack.kr(FFT(Buffer.alloc(s, 4096, 1), input), (filtre * 2).clip(0, 2));
					if(key < 12, freqin = (key + 60).midicps, freqin = (key - 12 + 60).midicps);
			...
			// Keyboard file
			SynthDef("OSC WekAgents File Keyboard",
			...
			SynthDef("OSC WekAgents Tempo AudioIn",
				{arg in=0, lock=0;
					...
					SendReply.kr(trackb, '/WekAgents_Analyse_Tempo', values: [tempo], replyID: [1]);
			}).send(s);

			// Analyse Tempo FileIn
			SynthDef("OSC WekAgents Tempo FileIn",
