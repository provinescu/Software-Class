...
Preset Wek",
		...
		s.waitForBoot({
			...
			~paraAlgoAnalyseAudio=[[0.5, 0.5], [0.5, 0.5]];
			~musicOutAgents=[];
			...
			~listemodeentree=['Audio','File', 'Off'];
			~sourceOutAgents=[];
			32.do({arg i;var x;
	...
	run {
			...
			if(File.exists(path++file), {file=File(path++file,"r");~sounds=file.readAllString.interpret.soloArray;file.close},
				{~sounds=
					[
						...
						"/Applications/SuperCollider.app/Contents/Resources/sounds/Buffer/sample1_2.aiff",
						"/Applications/SuperCollider.app/Contents/Resources/sounds/Buffer/sample1_4.aiff",
						"/Applications/SuperCollider.app/Contents/Resources/sounds/Buffer/sample1_8.aiff",
						"/Applications/SuperCollider.app/Contents/Resources/sounds/Buffer/sample1_16.aiff",
						"/Applications/SuperCollider.app/Contents/Resources/sounds/Buffer/sample1_32.aiff",
						"/Applications/SuperCollider.app/Contents/Resources/sounds/Buffer/sample1_64.aiff",
		...
		~creationSynthDefAnalyze={arg algo;
			s.bind{
				...
				switch(algo,
					...
					5, {
						~audioFile=Synth.newPaused("OSC WekAgents File MIDI",
							['busFileIn', ~busFileIn.index, \trigger, 0], ~groupeAnalyse, \addToTail);
						s.sync;
						~audioIn=Synth.newPaused("OSC WekAgents MIDI",
							[\trigger, 0], ~groupeAnalyse, \addToTail);
						s.sync;
						~tempoFile=Synth.newPaused("OSC WekAgents Tempo FileIn",
							['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
		...
		lastTimeWekData = Main.elapsedTime;
		...
		OSCFunc.newMatching({arg msg, time, addr, recvPort;
			...
			wekOut = msg[1..];
			{
				if(flagWTD == 'on' and: {(time - lastTimeWekData) > timeWekData}, {
					...
					~dureeInstr.valueAction = [wekOut[7], wekOut[8]].clip(0, 60);
					~dureeTempoinstr.value = wekOut[9].clip(-16, 60);
					~quantaMusicSlider.value = wekOut[10].clip(1, 100);
					// Genes
					~geneFreqRanger.valueAction = [wekOut[11], wekOut[12]].clip(0, 127);
					if(wekOut[13] <= 0.5,
					...
					~geneAmpRanger.valueAction = [wekOut[17], wekOut[18]].clip(-120, 0);
					if(wekOut[19] <= 0.5,
	...
	controlPanel {
		...
		~algoAnalyse = PopUpMenu(~wp,Rect(10, 10, 100, 20)).items=["Onsets","Pitch", "Pitch2", "KeyTrack", "Keyboard","MIDI"];
		...
		~viewpopulation = StaticText(~wp, Rect(10,10, 50, 20)).string_("Crew"+~agents.asString).stringColor_(Color.yellow);
		~viewagesagents = StaticText(~wp, Rect(10,10, 40, 20)).string_("Age"+~ages.mediane.asString).stringColor_(Color.yellow);
		...
		~viewsignaux = StaticText(~wp, Rect(10,10, 60, 20)).string_("Signal").stringColor_(Color.yellow);
		...
		~stateOSC = StaticText(~wp, Rect(10, 80, 40, 20)).string_("osc Off").stringColor_(Color.yellow);
		...
		~fonctionShortCuts = {arg window;
			...
			~evaluationKeyDown={arg char,modifiers,unicode, keycode, flagScore='off';
				...
				if(modifiers==0 and: {unicode==104} and: {keycode==4},{~keyCodeSourceIn = ~keyCodeSourceIn + 1;
					if(~keyCodeSourceIn > 2, {~keyCodeSourceIn=0});
						...
						if(~algoAnalyse.value == 1, {~algoAnalyse.valueAction_(2)}, {
							if(~algoAnalyse.value == 2, {~algoAnalyse.valueAction_(3)}, {
								if(~algoAnalyse.value == 3, {~algoAnalyse.valueAction_(4)}, {
									if(~algoAnalyse.value == 4, {~algoAnalyse.valueAction_(5)},
										if(~algoAnalyse.value == 5, {~algoAnalyse.valueAction_(0)});
									);
								});
							});
						});
	...
	initSynthDef {arg flag=true, f, w, x, y, z;
		...
		if(flag == true, {
			...
			// Keyboard file
			SynthDef("OSC WekAgents File MIDI",
				{arg busFileIn, note=60, amp=0.5, trigger = 0;
					var input, centroid, flatness, fft, energy, key, flux, array;
					input = In.ar(busFileIn);
					fft = FFT(LocalBuf(1024, 1), input);
			...
			SynthDef("DelayHarmonic",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					controlenvlevel1=0.0, controlenvlevel2=1.0, controlenvlevel3=1.0, controlenvlevel4=0.75, controlenvlevel5=0.75, controlenvlevel6=0.5, controlenvlevel7=0.5, controlenvlevel8=0.0,  controlenvtime1=0.015625, controlenvtime2=0.109375, controlenvtime3=0.25, controlenvtime4=0.25, controlenvtime5=0.125, controlenvtime6=0.125, controlenvtime7=0.125;
					var dureesample, envelope, ambisonic, main=0, maxDel=0.05, del, phase, envDel, inputSig;
					// Set Rate Freq
					rate=2**rate.cpsoct;
					dureesample=BufDur.kr(buffer)/rate;dureesample=dureesample+(loop*(duree-dureesample));dureesample=clip2(duree,dureesample);
					// envelope
