...
";
		...
		MainMenu.register(~menuFile.title_("File for Analyze"), "RobotBandTools");
		...
		MainMenu.register(~menuSynchro, "RobotBandTools");
		...
		MainMenu.register(~menuRec.title_("Recording"), "RobotBandTools");
		...
		MainMenu.register(~menuBuf, "RobotBandTools");
		...
		MainMenu.register(~menuAudio.title_("Audio"), "RobotBandTools");
		...
		MainMenu.register(~menuFX, "RobotBandTools");
		...
		MainMenu.register(~menuMidi.title_("MIDI"), "RobotBandTools");

		~menuShortCut = MenuAction("ShortCuts", {Document.new("ShortCuts for RobotBand", ~helpRobotBand)});
		MainMenu.register(~menuShortCut, "RobotBandTools");
		...
		~fonctionRecOn={
			...
			if(~flagRecording == 'off', {
				...
				s.bind{
					...
					s.recSampleFormat_(~sampleFormat);
					s.sync;
					s.prepareForRecord("~/Music/SuperCollider Recordings/".standardizePath ++ "RobotBand_" ++ Date.localtime.stamp ++ "." ++ ~headerFormat);
					s.sync;
					s.record;
					s.sync;
				};
	...
	run {
		...
		s.bind{
			...
			"Please Wait... Loading RobotBand... Sending SynthDef on Server... Loading Sounds...".postln;

			// OSC Synth Analyse
			~canalAudioIn = 0;
			//Creation Synth play File
			~synthPlayFile=Synth.newPaused("RobotBand Play File",
		...
		}, '/RobotBand_Analyse_Tempo', ~serverAdresse);
		// Fonction OSC
		~initOSCresponder = {
		...
		~foncseq={arg i, item; var file, datas, datasfile;
			if(item.value == 0 ,{nil},
				{if(item.value.odd, {item=item.value + 1 / 2;
					if(File.exists(~nompathdata++"sequence"+item.value.asString++".scd"),{file=File(~nompathdata++"sequence"+item.value.asString++".scd","r");datas=file.readAllString.interpret;file.close;
		...
		~foncpart={arg i, item; var file, datas, datasfile;
			if(item.value == 0 ,{nil},{if(File.exists(~nompathdata++"score"+item.value.asString++".scd"),{file=File(~nompathdata++"score"+item.value.asString++".scd","r");datas=file.readAllString.interpret;file.close;
				~listeplaypart.wrapPut(item.value-1,datas);
				~flagplaypart.wrapPut(item.value-1,0);
				~dureeplaypart.wrapPut(item.value-1,0);
			})});
		...
		~slcinstruments={arg i,item;
			...
			switch(item.value,
				...
				{3}, {Dialog.savePanel({arg path;
					...
					file=File(path++".scd","w");file.write(datasfile.value.asCompileString);file.close;~listewindow.wrapAt(i).name="RobotBand by HP Instrument"+(i+1).asString+path.asString},{"cancelled".postln});~saveloaddatasinstrument.wrapAt(i).value=0},
				{1}, {Dialog.openPanel({ arg paths;
					file=File(paths, "r");datas=file.readAllString.interpret;file.close;
					~fonctionloaddatasinstrument.value(i, datas.wrapAt(0),'on');~listewindow.wrapAt(i).name="RobotBand by HP Instrument"+(i+1).asString+paths.asString},{"cancelled".postln});~saveloaddatasinstrument.wrapAt(i).value=0},
				{4}, {Dialog.savePanel({arg path;
					datasfile=[];
					for(0, ~nombreinstrument-1, {arg ii;datasfile=datasfile.add(~fonctionsavedatasinstrument.value(ii));~listewindow.wrapAt(ii).name="RobotBand by HP Instrument"+(ii+1).asString+path.asString});
				...
				{12}, {if(File.exists(~nompathdata++"clear instruments.scd"),{file=File(~nompathdata++"clear instruments.scd", "r");datas=file.readAllString.interpret;file.close;
					16.do({arg canal; ~midiOut.allNotesOff(canal)});
					for(0, ~nombreinstrument-1, {arg ii;
						~fonctionloaddatasinstrument.value(ii, datas.wrapAt(ii),'on');~listewindow.wrapAt(ii).name="RobotBand by HP Instrument"+(ii+1).asString+"Clear Instruments"})});~saveloaddatasinstrument.wrapAt(i).value=0},
	...
	creationGUI {
		...
		~nombreinstrument.do({arg i;
			var w;
			w = Window("RobotBand by HP Instrument"+(i+1).asString, Rect(i*20, 200 - (i*33.33), 905, 525), scroll: true);
			w.alpha=1.0;w.front;
			...
			~loadsingleinstrument.wrapAt(i).action = {|item| var file, datas, datasfile;
				...
				if(item.value ==0 ,{nil},{if(File.exists(~nompathdata++"instrument"+item.value.asString++".scd"),{
					~flagfreq.wrapPut(i,0);~flagamp.wrapPut(i,0);~flagduree.wrapPut(i,0);~flagneuronefreq.wrapPut(i,0);~flagneuroneamp.wrapPut(i,0);~flagneuroneduree.wrapPut(i,0);~flagoutneuronefreq.wrapPut(i,0);~flagoutneuroneamp.wrapPut(i,0);~flagoutneuroneduree.wrapPut(i,0);~flaggenetiquefreq.wrapPut(i,0);~flaggenetiqueamp.wrapPut(i,0);~flaggenetiqueduree.wrapPut(i,0);~duree.wrapPut(i, 0);
					file=File(~nompathdata++"instrument"+item.value.asString++".scd","r");datas=file.readAllString.interpret;file.close;~fonctionloaddatasinstrument.value(i, datas.wrapAt(0),'on');~listewindow.wrapAt(i).name="RobotBand by HP Instrument"+(i+1).asString+"instrument"+item.value.asString++".scd"})});
			...
			~loadmultiinstruments.wrapAt(i).action = {|item| var file, datas, datasfile;
				...
				if(item.value ==0 ,{nil},{if(File.exists(~nompathdata++"instruments"+item.value.asString++".scd"),{file=File(~nompathdata++"instruments"+item.value.asString++".scd","r");datas=file.readAllString.interpret;file.close;
					...
					for(0, ~nombreinstrument-1, {arg ii;
						...
						~fonctionloaddatasinstrument.value(ii, datas.wrapAt(ii),'on');~listewindow.wrapAt(ii).name="RobotBand by HP Instrument"+(ii+1).asString+"instruments"+item.value.asString++".scd"})})});
		...
		// Control Panel
		~wg =Window("RobotBand by HP Control", Rect(0, 125, 305, 450), scroll: true);
		~wg.alpha=1.0;
		~wg.front;
			...
			switch(view.value,
				...
				1, {
					s.bind{
						...
						~audioFile.free;
						s.sync;
						~audioFile=Synth.newPaused("OSC RobotBand File Pitch",
							[\busFileIn, ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilanalyse.value], ~groupeAnalyse, \addToTail);
						s.sync;
						~audioIn=Synth.newPaused("OSC RobotBand Pitch",
							[\in, ~canalAudioIn, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilanalyse.value], ~groupeAnalyse, \addToTail);
						s.sync;
				...
				2, {
					s.bind{
						...
						~audioFile.free;
						s.sync;
						~audioFile=Synth.newPaused("OSC RobotBand File Pitch2",
							[\busFileIn, ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilanalyse.value], ~groupeAnalyse, \addToTail);
						s.sync;
						~audioIn=Synth.newPaused("OSC RobotBand Pitch2",
							[\in, ~canalAudioIn, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilanalyse.value], ~groupeAnalyse, \addToTail);
						s.sync;
						if(~flagEntreeMode == 'Audio IN' and: {~startsysteme.value == 1}, {~audioIn.value.run(true);~audioFile.value.run(false);~synthPlayFile.run(false);~tempoIn.value.run(true);~tempoFile.value.run(false)});
						s.sync;
				...
				3, {
					s.bind{
						...
						~audioFile.free;
						s.sync;
						~audioFile=Synth.newPaused("OSC RobotBand File KeyTrack",
							[\busFileIn, ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilanalyse.value], ~groupeAnalyse, \addToTail);
						s.sync;
						~audioIn=Synth.newPaused("OSC RobotBand KeyTrack",
							[\in, ~canalAudioIn, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilanalyse.value], ~groupeAnalyse, \addToTail);
						s.sync;
						if(~flagEntreeMode == 'Audio IN' and: {~startsysteme.value == 1}, {~audioIn.value.run(true);~audioFile.value.run(false);~synthPlayFile.run(false);~tempoIn.value.run(true);~tempoFile.value.run(false)});
						s.sync;
				...
				4, {
					s.bind{
						...
						~audioFile.free;
						s.sync;
						~audioFile=Synth.newPaused("OSC RobotBand Keyboard File", [\trigger, 0], ~groupeAnalyse, \addToTail);
						s.sync;
						~audioIn=Synth.newPaused("OSC RobotBand Keyboard", [\trigger, 0], ~groupeAnalyse, \addToTail);
						s.sync;
						if(~flagEntreeMode == 'Audio IN' and: {~startsysteme.value == 1}, {~audioIn.value.run(true);~audioFile.value.run(false);~synthPlayFile.run(false);~tempoIn.value.run(true);~tempoFile.value.run(false)});
						s.sync;
						if(~flagEntreeMode == 'File IN' and: {~startsysteme.value == 1}, {~audioFile.value.run(true);~synthPlayFile.run(true);~audioIn.value.run(false);~tempoIn.value.run(false);~tempoFile.value.run(true)});
...
~routineMetronome.play(quant: Quant.new(1)); nil});
		...
		~keysfonction={arg low,high, numinstr, f, d;
			...
			if(~commande=='savesingle',{if(high <= 40, {for(low,high,{arg i;
				...
				~listewindow.wrapAt(~instrumentactuel).name="RobotBand by HP Instrument"+(~instrumentactuel+1).asString+~nompathdata+"instrument"+i.asString++".scd"})});
			});
			//save instruments
			if(~commande=='savemulti',{if(high <= 40, {for(low,high,{arg i;
				...
				for(0, ~nombreinstrument-1, {arg ii;datasfile=datasfile.add(~fonctionsavedatasinstrument.value(ii));
					~listewindow.wrapAt(ii).name="RobotBand by HP Instrument"+(ii+1).asString+~nompathdata+"instruments"+i.asString++".scd"});
				datasfile=datasfile.add([~savecontrolpanel.value]);
	...
	initSynthDef {arg flag=true, outAmbisonic, w, x, y, z;
		...
		if(flag == true, {
			...
			// Liste des instruments pour RobotMusique version 2010

			// Audio In pour analyse

			SynthDef("OSC RobotBand Onsets",
			...
			SynthDef("OSC RobotBand KeyTrack",
			...
			// Keyboard Audio
			SynthDef("OSC RobotBand Keyboard",
				{arg in=0,note=60, amp=0.5, trigger = 0;
					...
					SendReply.kr(trigger, '/RobotBand_Analyse_Audio', values: [note, amp], replyID: [1, 2]);
			}).send(s);

			SynthDef("OSC RobotBand File Onsets",
			...
			SynthDef("OSC RobotBand File Pitch",
				{arg bufferplay, busFileIn,  seuil=0.125, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
					var input, detect, freqin, hasfreqin, ampin,
					inputFilter;
					input = In.ar(busFileIn);
			...
			SynthDef("OSC RobotBand File Pitch2",
			...
			SynthDef("OSC RobotBand File KeyTrack",
			...
			// Keyboard
			SynthDef("OSC RobotBand Keyboard File",
				{arg in=0,note=60, amp=0.5, trigger = 0;
					/*var input, centroid, flatness, fft, energy, key, flux;
					input= Mix(SoundIn.ar(in));
					fft = FFT(LocalBuf(2048, 1), input);
			...
			SynthDef("OSC RobotBand Tempo AudioIn",
				{arg in = 0, lock=0;
			...
			SynthDef("OSC RobotBand Tempo FileIn",
			...
			SynthDef("PlayBuf",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					pos = if(controls.at(0).value <= 0.01 , pos, Logistic.kr(controls.at(1)*4, 1, Rand(0, 1)));
					// Sample
					...
					Out.ar(buseffetsPost, main * amp * ampPost);
			...
			SynthDef("Spring*Buf",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					// Synth
					inforce=LFPulse.ar(controls.at(2)*100);
					damp=controls.at(3)*duree/10;
					freqspring=controls.at(4)*1000;
					main=SinOsc.ar(Spring.ar(inforce,duree/10/controls.at(5),damp)*freqspring)*in;
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("PlayBufAdd",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					envelope2=EnvGen.ar(Env.perc(controls.at(1),1,1,controls.at(2)*8-4),timeScale: dureesample, levelScale: controls.at(3), doneAction: 0);
					// Sample Attention les buffers doivent avoir la meme longueur !!!!!!!!!!!
					in1=PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate1, trigger, BufFrames.kr(buffer)*pos, loop) * envelope1;
					in2=PlayBuf.ar(1, buffer2, BufRateScale.kr(buffer2)*rate2, trigger, BufFrames.kr(buffer2)*controls.at(0), loop2) * envelope2;
					main=in1+in2;
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("Piano+Sample",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					// Sample Attention les buffers doivent avoir la meme longueur !!!!!!!!!!!
					in2=PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, 0, 0, loop) * envelope2;
					listefreq=listefreq.add(1/freq);listefreq=listefreq.add(1/(freq-(controls.at(3)*8)));listefreq=listefreq.add(1/(freq+(controls.at(4)*8)));
			...
			SynthDef("String+Sample",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					envelope2=EnvGen.ar(Env.linen(controls.at(7),controls.at(8),controls.at(9),1,4),timeScale: dureesample, levelScale: 1, doneAction: 0);
					// Sample Attention les buffers doivent avoir la meme longueur !!!!!!!!!!!
					in2=PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, 0, 0, loop2) * envelope2;
					listefreq=listefreq.add(1/freq);listefreq=listefreq.add(1/(freq-(controls.at(3)*8)));listefreq=listefreq.add(1/(freq+(controls.at(4)*8)));
			...
			SynthDef("SampleResonz",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					// Sample
					main=PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, trigger, BufFrames.kr(buffer)*pos, loop);
					main=Resonz.ar(main, XLine.ar(20000*controls.at(0)+27.5, 110*controls.at(1) + 27.5, controls.at(2)*dureesample));
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("WaveLoss",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,4), gate, timeScale: dureesample, levelScale: 1, doneAction: 2);
					// Sample
					main=WaveLoss.ar(PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, trigger, BufFrames.kr(buffer)*pos, loop), controls.at(0) * 40, 40, abs(controls.at(1) * 2 - 1));
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("FreqShift",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,4), gate, timeScale: dureesample, levelScale: 1, doneAction: 2);
					// Sample
					main=Mix(FreqShift.ar(PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, trigger, BufFrames.kr(buffer)*pos, loop), controls.at(0) * 512, controls.at(1) * 2pi));
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("Synthesizer",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					main = if(freq < 64.5.midicps , RLPF.ar(osc, XLine.ar(63.5.midicps*controls.at(0)+27.5, freq, dureesample*controls.at(1)), 0.333), RHPF.ar(osc, XLine.ar(127.midicps*controls.at(2)+27.5, freq, dureesample*controls.at(3)), 0.333));
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("HPplayBufVibrato",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					pos = if(controls.at(0).value <= 0.01 , pos, Logistic.kr(controls.at(1)*4, 1, Rand(0, 1)));
					// Sample
					main=PlayBuf.ar(1, buffer, SinOsc.kr(controls.at(2)*dureesample.reciprocal*10, mul: controls.at(3), add: rate), trigger, BufFrames.kr(buffer)*pos, loop);
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("Granulation",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					main = BufRd.ar(1, local, Phasor.ar(0, controls.at(2) * 4 - 2, 0, BufFrames.kr(local)), 1);
					BufWr.ar(DelayC.ar(in1, 1, controls.at(3)), local, Phasor.ar(0, controls.at(4) * 4 - 2, 0, BufFrames.kr(local)), 1);
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("Toupie",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					// Sample
					in1=PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, trigger, BufFrames.kr(buffer)*pos, loop);
					in2 = BufRd.ar(1, local, Phasor.ar(0, controls.at(2) * 2, 0, BufFrames.kr(local)), 1);
					main = in1 + in2;
					BufWr.ar(main, local, Phasor.ar(0, controls.at(3) * 2, 0, BufFrames.kr(local)), 1);
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("Centroid",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,4), gate, timeScale: dureesample, levelScale: 1, doneAction: 2);
					// Sample
					pos = if(controls.at(0).value <= 0.0 , pos, Logistic.kr(Rand(3, 4), 1, Rand(0, 1)));
					main=PlayBuf.ar(1, buffer, BufRateScale.kr(buffer)*rate, trigger, BufFrames.kr(buffer)*pos, loop);
					local =FFT(LocalBuf(2048, 1), main);
					local = SpecCentroid.kr(local);
					main = MidEQ.ar(main, local.unipolar, controls.at(1).clip(0.01, 1.0)*512/local.unipolar, controls.at(2).clip(0.01, 1.0)-0.5*128);
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("GrainBuf",
				{// listes de arguments (identique pour chaque sample !!!!!)
					...
					main=GrainBuf.ar(1, Dust.kr(100*controls.at(2)), controls.at(3)*0.1, buffer, rate, pos, 4, 0, -1, 512);
					//main = Limiter.ar(main, 1.0, 0.01);
			...
			SynthDef("Ringz_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.125, control2=0.125, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
						...
						if(~switchAudioOut == 'Multispeaker',
							...
							if(~switchAudioOut == 'Rotate2',
								...
								(ambisonic = PanB2.ar(effet, pan);
									DecodeB2.ar(~numberAudioOut, ambisonic[0], ambisonic[1], ambisonic[2])))));
					// Out
					Out.ar(buseffetsPost, Mix(effet));
					Out.ar(out, effet);
			}).send(s);
			...
			SynthDef("PitchShift_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.1, control2=0.1, control3=0.1, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree du effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("AllpassC_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.0625, control2=0.5, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
			...
			SynthDef("CombC_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.03, control2=0.5, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree de la effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("DelayC_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.3, control2=0, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree de la effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("DelayCtempo_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.3, control2=0, control3=0, control4=0, control5=0, control6=0, control7=0.60, control8=0.1, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
			...
			SynthDef("GVerb_pre",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, buseffetsPost, control1=0.0625, control2=0.5, control3=0.1, control4=0.1, control5=0.1, control6=0.1, control7=0.1, control8=0.1, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
			...
			SynthDef("Ringz_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.125, control2=0.125, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree du effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("PitchShift_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.1, control2=0.1, control3=0.1, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree du effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("AllpassC_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.0625, control2=0.5, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree de la effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("CombC_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.03, control2=0.5, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree de la effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("DelayC_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.3, control2=0, control3=0, control4=0, control5=0, control6=0, control7=0, control8=0, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic;
					// son en entree de la effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
			...
			SynthDef("DelayCtempo_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.3, control2=0, control3=0, control4=0, control5=0, control6=0, control7=0.60, control8=0.1, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic, tempo, beat, duree;
					tempo = (control7*100).floor;
					beat=(control8*100).floor;
			...
			SynthDef("GVerb_post",
				{// listes de arguments (identique pour chaque effet !!!!!)
					arg out=0, in, control1=0.3, control2=0.1, control3=0.1, control4=0.1, control5=0.1, control6=0.1, control7=0.1, control8=0.1, pan=0, amp=0;
					// liste des variables (identique pour chaque effet !!!!)
					var ineffet, effet, ambisonic, left, right;
					// son en entree de la effet + controles
					ineffet=Limiter.ar(Mix.new(In.ar(in,2)), 1.0, 0.01);
