...
MatrixBand {
	...
	*new	{arg path="~/Documents/MatrixBand/", o=2, r=2, f="Stereo", devIn="Built-in Microph", devOut="Built-in Output";
	...
	init	{arg path, o, r, f, devIn, devOut;
		...
		pathMatrixBand = PathName.new(path).pathOnly;

		// Verify Path
		if(File.exists(pathMatrixBand).not) {systemCmd("mkdir" + pathMatrixBand)};
	...
	run	{
		...
		fonctionCollectFolders = {
			...
			// Collect all Preset
			foldersToScanAll = PathName.new(pathMatrixBand).files.collect{ |path| var file;
				file = path.fileName;
...
";
		...
		MainMenu.register(menuFile.title_("File for Analyze"), "MatrixBandTools");
		...
		MainMenu.register(menuRecording.title_("Recording"), "MatrixBandTools");
		...
		MainMenu.register(menuAudio.title_("Audio"), "MatrixBandTools");
		...
		MainMenu.register(menuAlgo.title_("Automation"), "MatrixBandTools");
		...
		MainMenu.register(menuMIDI.title_("Midi"), "MatrixBandTools");
		...
		MainMenu.register(menuOSC.title_("OSC"), "MatrixBandTools");

		menuHelp = Menu(
			MenuAction("ShortCut", {
				Document.new("ShortCut for MatrixBand", helpMatrixBand);
			});
		);
			...
			item.value.switch(
				...
				3, {
					Dialog.openPanel({ arg paths;
						...
						f.close;
						windowControl.name="MatrixBand Control" + " | "+ PathName.new(paths).fileName;
						//Document.listener.string="";
						s.queryAllNodes;
						listeWindows.at(3).front;
						indexWindows=3;
				...
				6, {
					Dialog.savePanel({arg path;
						...
						f.close;
						windowControl.name="MatrixBand Control" + " | " + fileName},
					{"cancelled".postln});
				},
				// Copy Synthesizer
				7, {
		...
		fonctionRecOn={
			...
			if(flagRecording == 'off', {
				...
				s.bind{
					...
					s.recSampleFormat_(sampleFormat);
					s.sync;
					s.prepareForRecord("~/Music/SuperCollider Recordings/".standardizePath ++ "MatrixBand_" ++ Date.localtime.stamp ++ "." ++ headerFormat);
					s.sync;
					s.record;
					s.sync;
				};
		...
		s.waitForBoot({
			...
			s.bind{
				...
				// Synth Analyze In
				synthAnalyzeIn=Synth.newPaused("MatrixBand AnalyzeIn",
					[\in, 0, 'busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;

				//// Synth AudioIn
				//synthAudioIn=Synth.new("MatrixBand AudioIn",
				// [\in, 0, 'busIn', listeBusInOut.at(0)], groupeAnalyse, \addToTail);
				//s.sync;

				// Synth play file
				synthFileIn=Synth.newPaused("MatrixBand Play File",
				...
				synthAnalysePitch = Synth.newPaused("OSC MatrixBand Pitch",
					['busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;

				// Synth audio analyze Pitch2
				synthAnalysePitch2 = Synth.newPaused("OSC MatrixBand Pitch2",
					['busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;

				// Synth audio analyze Onsets
				synthAnalyseOnsets = Synth.newPaused("OSC MatrixBand Onsets",
					['busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;

				// Synth audio analyze KeyTrack
				synthAnalyseKeyTrack = Synth.newPaused("OSC MatrixBand KeyTrack",
					['busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;

				// Synth Keyboard
				synthKeyboard = Synth.newPaused("OSC MatrixBand Keyboard",
					['busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;

				// MIDI Keyboard
				midiKeyboard = Synth.newPaused("OSC MatrixBand MIDI",
					['busAnalyze', busAnalyze.index], groupeAnalyse, \addToTail);
				s.sync;
			...
			oscMusicData = OSCFunc.newMatching({arg msg, time, addr, recvPort;
				...
				if(msg.at(2) == 8 and: {flagKeyboard == 'off'},
					...
					{nil});
			}, '/MatrixBand_Musical_Data', serverAdresse);
			...
			oscKeyboardData = OSCFunc.newMatching({arg msg, time, addr, recvPort;
				...
				if(msg.at(2) == 8 and: {flagKeyboard == 'on'},
					...
					{nil});
			}, '/MatrixBand_Keyboard_Data', serverAdresse);
			...
			oscMIDIdata = OSCFunc.newMatching({arg msg, time, addr, recvPort;
				if(msg.at(2) == 5 and: {flagKeyboard == 'on'},
					...
					{nil});
			}, '/MatrixBand_MIDI_Data', serverAdresse);
						...
						if(onOffSynth == 1 and: {flagCollectFolders == 'off'}
							and: {meanProbaPresetFlux < (q1U - ecartqU) or: {meanProbaPresetFlux > (q3U + ecartqU)}} and: {meanProbaPresetFlatness < (q1A - ecartqA) or: {meanProbaPresetFlatness > (q3A + ecartqA)}} and: {abs(newTime - lastTimeAutomationPreset) > limitTemps},
							{
								...
								if(number != nil, {
									...
									if(File.exists(pathMatrixBand ++ foldersToScanAll.wrapAt(number)),
										{listeWindowSynth.do({arg window; window.close});
											file=File(pathMatrixBand ++ foldersToScanAll.wrapAt(number),"r");
											windowControl.name="MatrixBand Control" + " | " + foldersToScanAll.wrapAt(number);
	...
	createGUI  {
		...
		fonctionShortCut = {arg window;
			window.view.keyDownAction = {arg view,char,modifiers,unicode, keycode;
				...
				if(char == $k, {
					...
					flagCollectFolders = 'on';
					Dialog.openPanel({arg paths;
						pathMatrixBand= PathName.new(paths);
						pathMatrixBand = pathMatrixBand.pathOnly;
						windowControl.name="MatrixBand Control" + " | " + pathMatrixBand.asString;
						fonctionCollectFolders.value;
					}, {flagCollectFolders = 'off'});
				});
				...
				if(modifiers==131072 and: {unicode==67} and: {keycode==8} and: {window.name.containsStringAt(0, "MatrixBand Control").not} and: {window.name.containsStringAt(0, "MasterFX").not} and: {window.name.containsStringAt(0, "Master Sliders Music Control Synthesizer and FX").not}, {
				...
				if(modifiers==0 and: {unicode==112} and: {keycode==35} and: {window.name.containsStringAt(0, "MatrixBand Control").not} and: {window.name.containsStringAt(0, "MasterFX").not} and: {window.name.containsStringAt(0, "Master Sliders Music Control Synthesizer and FX").not}, {
					window.view.children.at(0).valueAction_(1);
				});
				// Key alt + p -> Stop Synth on front
				if(modifiers==524288 and: {unicode==112} and: {keycode==35} and: {window.name.containsStringAt(0, "MatrixBand Control").not} and: {window.name.containsStringAt(0, "MasterFX").not} and: {window.name.containsStringAt(0, "Master Sliders Music Control Synthesizer and FX").not}, {
				...
				if(char == $z, {
					...
					if(File.exists(pathMatrixBand ++ foldersToScanPreset.at(number)),
						{listeWindowSynth.do({arg window; window.close});
							file=File(pathMatrixBand ++ foldersToScanPreset.at(number),"r");
							windowControl.name="MatrixBand Control" + " | " + foldersToScanPreset.at(number);
				...
				if(char == $Z, {
					...
					if(File.exists(pathMatrixBand ++ foldersToScanSynthesizer.at(number)),
						{listeWindowSynth.do({arg window; window.close});
							file=File(pathMatrixBand ++ foldersToScanSynthesizer.at(number),"r");
							windowControl.name="MatrixBand Control" + " | " + foldersToScanSynthesizer.at(number);
		...
		fonctionCommandes = {arg window, commandeExecute, number;
			...
			if(commandeExecute == 'Save Preset',{
				...
				file=File(pathMatrixBand ++ "Preset" + number.asString ++ ".scd", "w");
				file.write(fonctionSavePreset.value(listeWindowSynth).asCompileString);
				file.close;
			});
			//load Preset
			if(commandeExecute == 'Load Preset',{
				if(File.exists(pathMatrixBand ++ "Preset" + number.value.asString ++ ".scd"),
					{fonctionUserOperatingSystem.value(9);
						windowControl.name="MatrixBand Control" + " | " + "Preset" + number.asString;
						file=File(pathMatrixBand ++ "Preset" + number.value.asString ++ ".scd", "r");
						fonctionLoadPreset.value(file.readAllString.interpret);
						file.close;listeWindows.at(3).front;indexWindows=3;
				}, {"cancelled".postln});
			});
			...
			if(commandeExecute == 'Save OSCmusicData', {
				file=File(pathMatrixBand ++ "OSCmusicData" + number.asString ++ ".scd", "w");
				file.write(listeDataOSC.asCompileString);
				file.close;
			});
			//Load OSCmusicData
			if(commandeExecute == 'Load OSCmusicData', {
				if(File.exists(pathMatrixBand ++ "OSCmusicData" + number.value.asString ++ ".scd"),
					{file=File(pathMatrixBand ++ "OSCmusicData" + number.value.asString ++ ".scd", "r");
		...
		////////////////////////////// Control Panel //////////////////////
		windowControl =Window("MatrixBand Control", Rect(300, 0, 410, 505), scroll: true);
		...
		fonctionWindowSynth={arg name, groupe, synthNumber, bufferOne, bufferTwo, canalIn;
			...
			knobPreLevel1 = EZSlider(windowSynth, 95 @ 15, "Pre", ControlSpec(0, 1, \lin, 0), {|ez| bufferRecording1.set(\preLevel, ez.value); ctrlBuffer.put(0, ez.value)}, 1, labelWidth: 30, numberWidth: 30);
			knobPostLevel1 = EZSlider(windowSynth, 95 @ 15, "Post", ControlSpec(0, 1, \lin, 0), {|ez| bufferRecording1.set(\postLevel, ez.value); ctrlBuffer.put(1, ez.value)}, 0, labelWidth: 30, numberWidth: 30);
	...
	initSynthDef  {
		...
		// MatrixBand Audio Analyze Onsets
		SynthDef("OSC MatrixBand Onsets",
			{arg busAnalyze, seuil=0.5, filtre=0.5, lock=0, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
				...
				SendReply.kr(detect, '/MatrixBand_Musical_Data', values: [freqIn, ampIn, timeIn, tempo, centroid, flatness.max(-100), energy, flux.max(-100)], replyID: [1, 2, 3, 4, 5, 6, 7, 8]);
		}).add;

		// MatrixBand Audio Analyze Pitch
		SynthDef("OSC MatrixBand Pitch",
		...
		// MatrixBand Audio Analyze Pitch2
		SynthDef("OSC MatrixBand Pitch2",
			{arg busAnalyze, seuil=0.5, filtre=0, lock=0, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
		...
		// MatrixBand Audio Analyze KeyTrack
		SynthDef("OSC MatrixBand KeyTrack",
		...
		// MatrixBand Keyboard
		SynthDef("OSC MatrixBand Keyboard",
			{arg busAnalyze, seuil=0.5, filtre=0, lock=0, note=0, amp=0, trigger=0;
				...
				SendReply.kr(trigger, '/MatrixBand_Keyboard_Data', values: [note, amp, timeIn, tempo, centroid, flatness.max(-100), energy, flux.max(-100)], replyID: [1, 2, 3, 4, 5, 6, 7, 8]);
		}).add;

		// MatrixBand MIDI
		SynthDef("OSC MatrixBand MIDI",
			{arg busAnalyze, seuil=0.5, filtre=0, lock=0, trigger=0;
				...
				flux =  FFTFlux.kr(fft);
				# trackB,trackH,trackQ, tempo = BeatTrack.kr(FFT(LocalBuf(1024, 1), input), lock);
				SendReply.kr(trigger, '/MatrixBand_MIDI_Data', values: [tempo, centroid, flatness.max(-100), energy, flux.max(-100)], replyID: [1, 2, 3, 4, 5]);
		}).add;

		// Synth pour analyse AudioIn send audio -> busIn
		SynthDef("MatrixBand AudioIn",
			{arg in=0, busIn;
				var input;
		...
		SynthDef('PV_HPshiftDown',
			{arg out=0, busIn, busOut, busFXout, busFXin, bufferOne, bufferTwo, loopOne=0, loopTwo=0, recBuffer1, recBuffer2, offset1, offset2, reverse1, reverse2,
				...
				buffer = if(switchBuffer1.value > 0, bufferOne, recBuffer1);
				// Synth
				chain = PlayBuf.ar(1, buffer, BufRateScale.kr(buffer) * rate * reverse1, 1, BufFrames.kr(buffer) * offset1, loop: loopOne);
		...
		SynthDef('DJ_FX',
			{arg out=0, busIn, busOut, busFXout, busFXin, bufferOne, bufferTwo, loopOne=0, loopTwo=0, recBuffer1, recBuffer2,
				...
				//FX
				chain = PlayBuf.ar(1, bufferOne, LFNoise2.kr(ctrl2.reciprocal) + (ctrl1*4), Dust.kr(ctrl3.reciprocal), Logistic.kr(ctrl4 / 2 + 3.5, 100, Rand(0, 1)) * BufFrames.kr(bufferOne), 1, ctrlHP1, ctrlHP2) + local * amp;
				// chain = Limiter.ar(chain, 1.0, 0.01);
		...
		SynthDef('PV_MagFreeze',
			{arg out=0, busIn, busOut, busFXout, busFXin, bufferOne, bufferTwo, loopOne=0, loopTwo=0, recBuffer1, recBuffer2,
				...
				chain = PlayBuf.ar(1, bufferOne, (ctrl1 * 2).clip(0.0625, 2.0) * reverse, 1.0, ctrl3 * BufFrames.kr(bufferOne), 1, ctrlHP1, ctrlHP2);
				chain = FFT(LocalBuf(2048, 1), chain);
		...
		SynthDef('PV_PlayBuf',
			{arg out=0, busIn, busOut, busFXout, busFXin, bufferOne, bufferTwo, loopOne=0, loopTwo=0, recBuffer1, recBuffer2,
				...
				in = Mix(In.ar(busFXin) + (In.ar(busIn) * levelLocalIn));
				RecordBuf.ar(in, bufferOne, loop: 1);
				chain = PlayBuf.ar(1, bufferOne, (ctrl1 * 2).clip(0.0625, 2.0) * reverse, 1.0, 0, 1, ctrlHP1, ctrlHP2);
				chain = FFT(LocalBuf(512, 1), chain);
				PV_RecordBuf(chain, local, 0, 1, 1);
