...
MultiMatrix {
	...
	var degrees, root, scale, tuning, automationSliderTrans, automationSliderStretch, automationSliderQuant, automationSliderPan, automationSliderAmp, clientID;

	*new	{arg path="~/Documents/Matrix/", ni=26, o=2, r=2, f=0, devIn="Built-in Microph", devOut="Built-in Output", size = 256, wid=2.0, ori=0.5, flag=0, name="Matrix", wek=6448, wekPort=57120, scPort=rrand(57600, 57700), id=rrand(0, 256);

		^super.new.init(name, path, ni, o, r, f, devIn, devOut, size, wid, ori, flag, scPort, id);

	}

	init	{arg name, path, ni, o, r, f, devIn, devOut, size, wid, ori, flag, scPort, id;
		...
		s = Server.new("MultiMatrix"++scPort.asString, NetAddr("localhost", scPort), Server.default.options);
...
";
		...
		s.waitForBoot({
			...
			s.bind{
				...
				synthAnalyzeIn=Synth.newPaused("Matrix AnalyzeIn",
					[\in, 0, 'busAnalyze', busAnalyze.index, \id, clientID], groupeAnalyse, \addToTail);
				s.sync;

				// Synth play file
				synthFileIn=Synth.newPaused("Matrix Play File",
					[\bufferplay, bufferFile, 'busIn', listeBusInOut.at(0), 'busAnalyze', busAnalyze.index, \volume, 0.0, \id, clientID], groupeAnalyse, \addToTail);
				s.sync;

				// Synth audio analyze Pitch1
				synthAnalysePitch = Synth.newPaused("OSC Matrix Pitch",
					['busAnalyze', busAnalyze.index, \id, clientID], groupeAnalyse, \addToTail);
				s.sync;
			...
			oscMusicData = OSCFunc.newMatching({arg msg, time, addr, recvPort;
				...
				if(msg.at(2) == clientID and: {flagKeyboard == 'off'},
					{
						// Music
						freq=msg.at(3);
						amp=msg.at(4);
			...
			oscKeyboardData = OSCFunc.newMatching({arg msg, time, addr, recvPort;
				var freq, amp, duree, tempo, freqCentroid=0, flatness=0, energy=0, flux=0, musicData=[];
				if(msg.at(2) == clientID and: {flagKeyboard == 'on'},
					{
						// Music
						freq=msg.at(3);
						amp=msg.at(4);
	...
	createGUI  {
		...
		// BPM System
		bpmSlider=EZSlider(windowControl, Rect(0, 0, 150, 15), "BPM", ControlSpec(1, 960, \exp, 0),
		...
		fonctionWindowSynth={arg name, groupe, synthNumber, bufferOne, bufferTwo, canalIn, timeBuf1, timeBuf2, freezeOSC;
			...
			fonctionEnabledSlider = {arg flagAuto=0;
				...
				if(flagAuto == 1, {
					automationSliderFreq.enabled_(true); automationSliderDur.enabled_(true); automationSliderSynth.enabled_(true); automationNumberSynth.enabled_(true);automationSliderBuffer.enabled_(true);automationSliderTrans.enabled_(true); automationSliderStretch.enabled_(true); automationSliderQuant.enabled_(true); automationSliderPan.enabled_(true); automationSliderAmp.enabled_(true)}, {
					automationSliderFreq.enabled_(false); automationSliderDur.enabled_(false); automationSliderSynth.enabled_(false); automationNumberSynth.enabled_(false);automationSliderBuffer.enabled_(false);automationSliderTrans.enabled_(false); automationSliderStretch.enabled_(false); automationSliderQuant.enabled_(false); automationSliderPan.enabled_(false); automationSliderAmp.enabled_(false)});
				// Synthese
	...
	initSynthDef  {
		...
		SynthDef("OSC Matrix Onsets",
			{arg busAnalyze, seuil=0.5, filtre=0.5, lock=0, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0, id=0;
				...
				SendReply.kr(detect, '/Matrix_Musical_Data', values: [freqIn, ampIn, timeIn, tempo, centroid, flatness.clip(0.0001, 1), energy, flux.clip(0.0001, 1)], replyID: id);
		}).add;
		...
		SynthDef("OSC Matrix Keyboard",
			{arg busAnalyze, seuil=0.5, filtre=0, lock=0, note=0, amp=0, trigger=0, id=0;
				...
				SendReply.kr(trigger, '/Matrix_Keyboard_Data', values: [note, amp, timeIn, tempo, centroid, flatness.clip(0.0001, 1), energy, flux.clip(0.0001, 1)], replyID: id);
		}).add;

		// Matrix MIDI
		SynthDef("OSC Matrix MIDI",
			{arg busAnalyze, seuil=0.5, filtre=0, lock=0, trigger=0, id=0;
				...
				flux =  FFTFlux.kr(fft);
				# trackB,trackH,trackQ, tempo = BeatTrack.kr(FFT(LocalBuf(1024, 1), input), lock);
				SendReply.kr(trigger, '/Matrix_MIDI_Data', values: [tempo, centroid, flatness.clip(0.0001, 1), energy, flux.clip(0.0001, 1)], replyID: id);
		}).add;
