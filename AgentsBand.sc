...
";
		...
		MainMenu.register(~menuFile.title_("File for Analyze"), "AgentsBandTools");
		...
		MainMenu.register(~menuFX, "AgentsBandTools");
		...
		MainMenu.register(~menuRecording.title_("Recording"), "AgentsBandTools");
		...
		MainMenu.register(~menuAudio.title_("Audio"), "AgentsBandTools");
		...
		MainMenu.register(~menuAlgo.title_("Algorithm"), "AgentsBandTools");
		...
		MainMenu.register(~menuMidi.title_("MIDI"), "AgentsBandTools");
		...
		MainMenu.register(~menuSoft.title_("Soft"), "AgentsBandTools");

		~menuKeys = Menu(
			MenuAction("List ShortCuts AgentsBand", {Document.new("ShortCuts for AgentsBand",~helpHPAgentsBand)};
			),
		...
		MainMenu.register(~menuKeys.title_("ShortCuts"), "AgentsBandTools");
		...
		~fonctionRecOn={
			...
			if(~flagRecording == 'off', {
				...
				s.bind{
					...
					s.sync;
					s.prepareForRecord("~/Music/SuperCollider Recordings/".standardizePath ++ "AgentsBand_" ++ Date.localtime.stamp ++ "." ++ ~headerFormat);
					s.sync;
					s.record;
					s.sync;
				};
	...
	run {
		...
		~initAllBuffer={arg file;
			...
			s.sync;
			"Please Wait... Loading AgentsBand... Sending SynthDef on Server... Loading Sounds...".postln;
		};
		~initAllBuffer.value;
		...
		~audioFile=Synth.newPaused("OSC AgentsBand File Onsets",
			['busFileIn', ~busFileIn.index], ~groupeAnalyse, \addToTail);
		s.sync;
		~audioIn=Synth.newPaused("OSC AgentsBand Onsets",
			[\seuil, 0.5 ], ~groupeAnalyse, \addToTail);
		s.sync;
		// Creation Synth Tempo
		~tempoFile=Synth.newPaused("OSC AgentsBand Tempo FileIn",
			['busFileIn', ~busFileIn.index], ~groupeAnalyse, \addToTail);
		s.sync;
		~tempoIn=Synth.newPaused("OSC AgentsBand Tempo AudioIn",
		...
		~creationSynthDefAnalyze={arg algo;
			s.bind{
				...
				s.sync;
				switch(algo,
					0, {~audioFile=Synth.newPaused("OSC AgentsBand File Onsets",
					...
					1, {~audioFile=Synth.newPaused("OSC AgentsBand File Pitch",
					...
					s.sync;
					~audioIn=Synth.newPaused("OSC AgentsBand Pitch",
						[\seuil, ~seuilInSlider.value, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1)], ~groupeAnalyse, \addToTail);
					s.sync;
					~tempoFile=Synth.newPaused("OSC AgentsBand Tempo FileIn",
						['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
					...
					2, {~audioFile=Synth.newPaused("OSC AgentsBand File Pitch2",
						[\bufferplay, ~bufferanalysefile.bufnum, 'busFileIn', ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilInSlider.value], ~groupeAnalyse, \addToTail);
					s.sync;
					~audioIn=Synth.newPaused("OSC AgentsBand Pitch2",
						[\seuil, ~seuilInSlider.value, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1)], ~groupeAnalyse, \addToTail);
					s.sync;
					~tempoFile=Synth.newPaused("OSC AgentsBand Tempo FileIn",
						['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
					...
					3, {~audioFile=Synth.newPaused("OSC AgentsBand File KeyTrack",
						[\bufferplay, ~bufferanalysefile.bufnum, 'busFileIn', ~busFileIn.index, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1), \seuil, ~seuilInSlider.value], ~groupeAnalyse, \addToTail);
					s.sync;
					~audioIn=Synth.newPaused("OSC AgentsBand KeyTrack",
						[\seuil, ~seuilInSlider.value, \filtre, ~paraAlgoAnalyseAudio.wrapAt(~algoAnalyse.value).wrapAt(1)], ~groupeAnalyse, \addToTail);
					s.sync;
					~tempoFile=Synth.newPaused("OSC AgentsBand Tempo FileIn",
						['busFileIn', ~busFileIn.index, \lock, 0], ~groupeAnalyse, \addToTail);
					...
					4, {~audioFile=Synth.newPaused("OSC AgentsBand File Keyboard",
						['busFileIn', ~busFileIn.index, \trigger, 0], ~groupeAnalyse, \addToTail);
					s.sync;
					~audioIn=Synth.newPaused("OSC AgentsBand Keyboard",
		...
		~synthPlayFile=Synth.newPaused("AgentsBand Play File",
		...
		})}, '/AgentsBand_Analyse_Tempo', ~serverAdresse);
		// Fonction OSC
		~initOSCresponder = {
				...
				{duree=time - ~lastTimeAnalyse;// duree reel
					if(duree > ~tempsmaxsignal , // ici duree silence
						{~listefreq=[];~listeamp=[];~listeduree=[];~listeID=[]; ~lastTimeAnalyse=time;~freqBefore=0;~ampBefore=0;~dureeBefore=0;~freqTampon=nil;~ampTampon=nil;~compteurAnalyse=0;
							~agents.do({arg agent; ~listeagentID.wrapPut(agent, []);if(~listeagentfreq.wrapAt(agent).size != 0 ,{~flagplayagent.wrapPut(agent, 'new');~dureesmusique.wrapPut(agent, ~quantaMusic.reciprocal);~routineMusic.wrapAt(agent).reset})}); ~lastTimeAnalyse=time})});
		}, '/AgentsBand_Analyse_Audio', ~serverAdresse);
		...
		~agentsmusique={arg agent;
			...
			if(~flagGeneEnvDuree == 'off',
				{nil},
				{timeEnv =~genomes.wrapAt(agent).copyRange(26, 33).sort;
			...
			//Amp
			if(~flagAmpSynth == 'off', {amp = amp * ampRange + ampLow; ampReal = amp}, {ampReal = amp * ampRange + ampLow; amp= amp * 0});
			// Duree
			...
			freq = freq.soloArray;
			amp = amp.soloArray;
			ampReal = ampReal.soloArray;
			// Playing
			freq.size.do({arg index;var a, ar=0;
				// Amp
				a = amp.wrapAt(index);
				ar = ampReal.wrapAt(index);
				// Send MIDI On
				if(~flagMidiOut == 'on' and: {~canalMidiOutAgent.wrapAt(agent) >= 0}, {
	...
	controlPanel {
		...
		StaticText(~wp, Rect(0, 0, 500, 20)).string_("AgentsBand by Provinescu's Software Production").stringColor_(Color.white).font_(Font("Georgia", 12)).align = \right;
		~wp.view.decorator.nextLine;
		StaticText(~wp, Rect(10,10,500,20)).string_("System AgentsBand").stringColor_(Color.white).font_(Font("Georgia-BoldItalic", 12));
		~wp.view.decorator.nextLine;
	...
	shortCuts {
		...
		~fonctionShortCuts = {arg window;
			window.view.keyDownAction = {arg view,char,modifiers,unicode, keycode;
				if(view.asString != "a SCTextView", {
					...
					if(unicode != 21 and: {keycode != 32}, {~fonctionRecordScore.value("~evaluationKeyDown", [char,modifiers,unicode, keycode, 'on'])});
		...
		~foncSaveUnivers={arg datafile, flagGenome='off', flagSequence='off';//herite de foncSave monde
			...
			// + Genome
			if(flagGenome == 'on', {datafile=datafile.add(~genomes)});//205
			// + Sequence
			if(flagSequence == 'on', {datafile=datafile.add([~listeagentfreq,~listeagentamp,~listeagentduree])});//206
			datafile.value;
		};
		...
		~loadUnivers={arg file, flagGenome='off', flagSequence='off';
			...
			// + Genome (205) + Sequence (206)
			if(flagGenome == 'on' and: {datafile.wrapAt(205).size != 0}, {
				~genomes=datafile.wrapAt(205);// Load Genomes
				~startpopulation = ~agents = ~genomes.size;
				~foncInitAgents.value;
				~genomes=datafile.wrapAt(205); // Load Genomes again for INIT
				if(flagSequence == 'on' and: {datafile.wrapAt(206).wrapAt(0).size != 0}, {sequence=datafile.wrapAt(206);~listeagentfreq=sequence.wrapAt(0);~listeagentamp=sequence.wrapAt(1);~listeagentduree=sequence.wrapAt(2)}, {flagSequence='off'})},
			{~foncInitAgents.value});
		};
	...
	initSynthDef {arg flag=true, f, w, x, y, z;
		...
		if(flag == true, {
			...
			// Synth pour systeme AgentsBand

			// New Analyse Audio
			SynthDef("OSC AgentsBand Onsets",
				{arg in=0,  seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
					...
					SendReply.kr(detect, '/AgentsBand_Analyse_Audio', values: [freqin, ampin, centroid, flatness.max(-100), energy, flux.max(-100)], replyID: [1, 2, 3, 4, 5, 6]);
			}).send(s);

			// New Analyse Audio  2
			SynthDef("OSC AgentsBand Pitch",
			...
			// New Analyse Audio  3
			SynthDef("OSC AgentsBand Pitch2",
				{arg in=0, seuil=0.5, filtre=0, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
			...
			// New Analyse Audio  4
			SynthDef("OSC AgentsBand KeyTrack",
			...
			// Keyboard
			SynthDef("OSC AgentsBand Keyboard",
				{arg in=0, note=60, amp=0.5, trigger = 0;
					...
					SendReply.kr(trigger, '/AgentsBand_Analyse_Audio', values: [note, amp, centroid, flatness.max(-100), energy, flux.max(-100)], replyID: [1, 2, 3, 4, 5, 6]);
			}).add;

			// New Analyse File
			SynthDef("OSC AgentsBand File Onsets",
			...
			SynthDef("OSC AgentsBand File Pitch",
			...
			SynthDef("OSC AgentsBand File Pitch2",
				{arg  busFileIn, seuil=0.5, filtre=0, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
			...
			SynthDef("OSC AgentsBand File KeyTrack",
			...
			// Keyboard file
			SynthDef("OSC AgentsBand File Keyboard",
			...
			SynthDef("OSC AgentsBand Tempo AudioIn",
				{arg in=0, lock=0;
			...
			SynthDef("OSC AgentsBand Tempo FileIn",
			...
			SynthDef("MdaPiano",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,'sine'), 1.0, timeScale: duree, levelScale: 1.0, doneAction: 2);
					// Synth Piano
					main = Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25));
					// main = Limiter.ar(main, 1.0, 0.01);
					//ampreal = if(amp <= 0, ampreal, amp);
					// Switch Audio Out
					main = if(~switchAudioOut == 'Stereo',
			...
			SynthDef("Piano Synthesizer",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					// Main Synth
					freq = freq.clip(20,20000);
					osc = Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25));
			...
			SynthDef("Piano Squiz",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,'sine'), 1.0, timeScale: duree, levelScale: 1.0, doneAction: 2);
					// Main Synth
					main=Squiz.ar(Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25)), controlF * 10, controlA * 10);
					// main = Limiter.ar(main, 1.0, 0.01);
					//ampreal = if(amp <= 0, ampreal, amp);
					// Switch Audio Out
					main = if(~switchAudioOut == 'Stereo',
			...
			SynthDef("Piano WaveLoss",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,'sine'), 1.0, timeScale: duree, levelScale: 1.0, doneAction: 2);
					// Main Synth
					main=WaveLoss.ar(Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25)), controlF* 40, 40, abs(controlF*2-1));
					// main = Limiter.ar(main, 1.0, 0.01);
					//ampreal = if(amp <= 0, ampreal, amp);
					// Switch Audio Out
					main = if(~switchAudioOut == 'Stereo',
			...
			SynthDef("Piano FreqShift",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,'sine'), 1.0, timeScale: duree, levelScale: 1.0, doneAction: 2);
					// Main Synth
					main=Mix(FreqShift.ar(Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25)), controlF * 1024 - 512, controlA * 2pi));
					// main = Limiter.ar(main, 1.0, 0.01);
					//ampreal = if(amp <= 0, ampreal, amp);
					// Switch Audio Out
					main = if(~switchAudioOut == 'Stereo',
			...
			SynthDef("Piano PitchShift",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,'sine'), 1.0, timeScale: duree, levelScale: 1.0, doneAction: 2);
					// Main Synth
					main=Mix(PitchShift.ar(Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25)), 0.2, controlF*4, controlA, controlD, 1));
					// main = Limiter.ar(main, 1.0, 0.01);
					//ampreal = if(amp <= 0, ampreal, amp);
					// Switch Audio Out
					main = if(~switchAudioOut == 'Stereo',
			...
			SynthDef("Piano PV_Max",
				{arg out=0, buseffets, busverb, freq=0, rate=0, amp=0,  ampreal=0, duree=1.0, panLo=0, panHi=0, offset=0, loop=0, reverse=1, buffer, buffer2,
					...
					envelope=EnvGen.ar(Env.new([controlenvlevel1,controlenvlevel2,controlenvlevel3,controlenvlevel4,controlenvlevel5,controlenvlevel6,controlenvlevel7,controlenvlevel8],[controlenvtime1,controlenvtime2,controlenvtime3,controlenvtime4,controlenvtime5,controlenvtime6,controlenvtime7].normalizeSum,'sine'), 1.0, timeScale: duree, levelScale: 1.0, doneAction: 2);
					// Synth
					in1 = Mix(MdaPiano.ar(freq, gate: 1, vel: 127 * ampreal, hard: ampreal / 2 - 0.25));
					// RecordBuf
			...
			// Effets pour systeme AgentsBand
