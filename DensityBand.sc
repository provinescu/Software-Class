...
DensityBand {
	...
	*new {arg path = "~/Documents/DensityBand/", numberOut=2, numberRec=2, format="Stereo", devIn="Built-in Microph", devOut="Built-in Output";
	...
	init {arg path, numberOut, numberRec, format, devIn, devOut;
		...
		//server.options.hardwareBufferSize_(256);
		...
		fonctionLoadPreset = {arg allData, index=0;
			...
			listeWindows.do({arg window, data;
				...
				window.view.children.do({arg view, item;
					...
					if(item == 62, {autoRoot = data.at(item).at(1)});
			...
			// Setup AutoRoot
			windowEar.view.children.at(62).valueAction_(autoRoot);
			// synthOrchestra
	...
	run {
		...
		server.waitForBoot({
			...
			"Please Wait... Loading DensityBand... Sending SynthDef on s... Loading Sounds...".postln;
			...
			windowEar.view.children.at(62).enabled_(false);
			windowEar.view.children.at(81).valueAction_(0);// No Tuning
			windowEar.view.children.at(82).enabled_(false);
			windowEar.view.children.at(83).enabled_(false);
			windowEar.view.children.at(67).valueAction_(1);
			windowEar.view.children.at(68).valueAction_(1);
			windowEar.view.children.at(69).valueAction_(1);
			windowEar.view.children.at(70).valueAction_(1);
			windowEar.view.children.at(67).enabled_(false);
			windowEar.view.children.at(68).enabled_(false);
			...
			}, '/DensityBand_Music_Data', serveurAdresse);
			...
			computeAlgoFilterDataMusic = {arg freq, amp, duree, data, kohonenF, kohonenA, kohonenD, geneticF, geneticA, geneticD, neuralFAD, algorithm;
				...
				switch(algorithm,
					...
					'Kohonen', {
						...
						maxTraining = maxTraining.min(512);
			...
			buildSynth = {arg indexBandFhz;
				...
				if(indexBandFhz == 0,
					...
					{canalMidi = (indexBandFhz).mod(midiOutHi.asInteger) + 1};
				);
				// Envelope
				...
				if(flagRoot == 'on', {
					...
					// Setup GUI Value
					{windowEar.view.children.at(82).children.at(2).valueAction_(root);}.defer;
				});
				// MASTEROUT
				...
				{displayAnalyzeMusic.string = ("ADDED NEW SYNTHESIZER    " + " Instruments:" + listeDataInstruments.size + "    Algo: " + displayAlgo + "    FhzBand: " + displayIndex)}.defer;
			};
					...
					if((Main.elapsedTime - dureeAnalyzeOSCMusic) > memoryTime,
						{
							...
							dureeAnalyzeOSCMusic = Main.elapsedTime;
							// Display Analyze Music
							{displayAnalyzeMusic.string = ("SILENT (NO DATA ENTRY)    " + "    Instruments:" + listeDataInstruments.size)}.defer;
					});
					// Waiting Time
					//(1 - globalDensity + 0.01).wait;
					quantizationDuree.reciprocal.wait;
		...
		plotDataMusic = {arg freq, amp, duree;
			var analyzeData;
			{
				...
				// Display Analyze Music
				displayAnalyzeMusic.string = ("Freq:" + freq.cpsmidi.asStringPrec(4)  +  "   Amp:" + amp.ampdb.asStringPrec(4) + "   Duree:" + duree.asStringPrec(4) + "   Instruments:" + listeDataInstruments.size + "   Data:" +  dataFreq.at(0).size + "   Index:" + indexDataFreq.at(0) + "   Algo: " + displayAlgo + "   FhzBand: " + displayIndex);
			}.defer;
		};
...
";
		...
		MainMenu.register(menuFile.title_("File for Analyze"), "DensityBandTools");
		...
		menuPreset = Menu(
			MenuAction("Load Preset", {Dialog.openPanel({ arg path, file;
				...
				file.close;
				windowEar.name="DensityBand" + typeMasterOut + " | " + PathName.new(path).fileName;
			},
			{"cancelled".postln})}),
			MenuAction("Save Preset",{Dialog.savePanel({arg path, name, pathonly, fileName, file;
				...
				file.close;
				windowEar.name = "DensityBand" + typeMasterOut + " | " + fileName;
			}, {"cancelled".postln})});
		);
		MainMenu.register(menuPreset.title_("Preset"), "DensityBandTools");
		...
		MainMenu.register(menuInitAll.title_("Synth/Sound/FX"), "DensityBandTools");
		...
		MainMenu.register(menuRecording.title_("Recording"), "DensityBandTools");
		...
		MainMenu.register(menuMIDI.title_("Midi"), "DensityBandTools");
		...
		menuOSC = Menu(
			...
			MenuAction("OSC Master", {oscStateFlag='master';
				"DensityBand is OSC now !".postln;
			}),
			MenuAction("OSC Slave", {oscStateFlag='slave';
				"DensityBand is OSC SLAVE now !".postln;
			}),
			MenuAction("OSC Off", {oscStateFlag='off';
				"OSC is OFF now !".postln;
			});
		);
		MainMenu.register(menuOSC.title_("OSC"), "DensityBandTools");

		menuAlgo = Menu(
			MenuAction("Not Activate", {nil}),
		);
		MainMenu.register(menuAlgo.title_("Algorithm"), "DensityBandTools");

		menuHelp = MenuAction("Help ShortCut", {Document.new("ShortCut for DensityBand", helpDensityBand)});
		MainMenu.register(menuHelp, "DensityBandTools");
			...
			window.view.keyDownAction = {arg view,char,modifiers,unicode, keycode, number, file;
				...
				if(char == $z, {number = rrand(0, foldersToScanPreset.size - 1);
					if(File.exists(pathData ++ foldersToScanPreset.at(number)),
						{file=File(pathData ++ foldersToScanPreset.at(number),"r");
							windowEar.name = "DensityBand" + typeMasterOut + " | " + foldersToScanPreset.at(number);
							fonctionLoadPreset.value(file.readAllString.interpret);
				...
				if(char == $k, {
					FileDialog.new({arg path;
						...
						windowEar.name="DensityBand" + " | " + pathData.asString;
						fonctionCollectFolders.value;
					}, fileMode: 0);
				});
			};
		...
		fonctionShortCutCommande = {arg number, file;
			if(shortCutCommande == 'Save Preset', {
				windowEar.name = "DensityBand" + " | " + "Preset" + number.asString;
		...
		/////// DensityBand Ear Panel
		windowEar = Window("DensityBand" + typeMasterOut, Rect(0, 800, 715, 825), scroll: true);
		windowEar.alpha=1.0;
		windowEar.front;
		...
		EZRanger(windowEar , 250 @ 20, "Canals", ControlSpec(1, 16, \lin, 1),
			{|ez| midiOutLo = ez.value.at(0); midiOutHi = ez.value.at(1)}, [1, 16], labelWidth: 50, numberWidth: 25);
		windowEar.view.decorator.nextLine;
		...
		EZKnob(windowEar, 150 @ 20, "FhzBand", ControlSpec(1, 12, \lin, 1),
			{|ez| numFhzBand = ez.value;
				...
				for(0, numFhzBand,
					{arg index;
						windowEar.view.children.at(67 + index).enabled_(true);
				});
				if(numFhzBand < 12, {
					for(numFhzBand + 1, 12,
						{arg index;
							windowEar.view.children.at(67 + index).enabled_(false);
							windowEar.view.children.at(67 + index).valueAction_(0);
					});
				});
				numIndexSynthBand = 0
		}, 3, layout: \horz);
		...
		// HP Threshold
		EZKnob(windowEar, 80 @ 80, "Threshold", ControlSpec(0.01, 1, \lin, 0.01),
		...
		MultiSliderView(windowEar, Rect(0, 0, 35, 70)).value_([1, 0]).action={arg levels;
			recLevel = levels.value.at(0); preLevel = levels.value.at(1); groupeRecBuffer.set(\level1, recLevel, \level2, preLevel); groupeSynth.set(\level1, recLevel, \level2, preLevel)};
		windowEar.view.decorator.nextLine;
		// Range Pan
		...
		action_({arg loop; if(loop.value == 0, {loopSound = 0;
			// Setup GUI Value
			if(windowEar.view.children.at(48).value == 1, {windowEar.view.children.at(48).valueAction = 0})}, {loopSound = 1})});
		// Reverse Sound
		Button(windowEar,Rect(0, 0, 100, 20)).
		...
		action_({arg loop; if(loop.value == 0, {loopRec = 0}, {loopRec = 1;
			// Setup GUI Value
			if(windowEar.view.children.at(45).value == 0, {windowEar.view.children.at(45).valueAction = 1})})});
		windowEar.view.decorator.nextLine;
		// Display Instrument
		...
		Button(windowEar, Rect(0, 0, 90, 20)).states_([["AutoRoot On", Color.green], ["AutoRoot Off", Color.red]]).action_({|view|
			if(view.value == 1, {flagRoot = 'on';
				...
				// Setup GUI Value
				windowEar.view.children.at(82).enabled_(true);
				windowEar.view.children.at(82).children.at(1).valueAction_(12);
				windowEar.view.children.at(82).children.at(1).valueAction_(0);
			};
			);
		});
		// Automation BPM
		...
		action = {arg item;
			// Setup GUI Value
			//windowEar.view.children.at(62).valueAction_(0);
			windowEar.view.children.at(82).children.at(1).valueAction_(12);
			windowEar.view.children.at(82).children.at(1).valueAction_(0);
			windowEar.view.children.at(62).enabled_(true);
			windowEar.view.children.at(82).enabled_(true);
			windowEar.view.children.at(83).enabled_(true);
			switch(item.value,
				// No Scale
				0, {flagScaling = 'off';
					// Setup GUI Value
			...
			if(item.value > 1 and: {item.value < 28}, {tuning = Tuning.et12; scale = Scale.new(((degrees + root)%tuning.size).sort, tuning.size, tuning); flagScaling = 'on';
				// Setup GUI Value
				windowEar.view.children.at(83).children.at(1).valueAction = degrees.asString;
			});
	...
	initSynthDef {
		...
		// DensityBand Audio Analyze FFT
		SynthDef("OSC DensityBand FFT",
			{arg busIn, speed=24;
				...
				SendReply.kr(Impulse.kr(speed), '/DensityBand_FFT_Data', values: [flux.clip(0.0001, 1), flatness.clip(0.0001, 1), centroid.clip(20, 20000), energy.clip(20, 20000), bpm], replyID: [1, 2, 3, 4, 5]);
		}).add;

		// DensityBand Audio Analyze Onsets
		SynthDef("OSC DensityBand Onsets",
			{arg busIn, seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
				...
				SendReply.kr(detect, '/DensityBand_Music_Data', values: [freqIn, ampIn, timeIn], replyID: [1, 2, 3]);
		}).add;

		// DensityBand Audio Analyze Pitch
		SynthDef("OSC DensityBand Pitch",
			{arg busIn, seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
				var input, detect, freqIn, hasfreqIn, ampIn, timeIn=0,
				inputFilter;
				input = In.ar(busIn);
		...
		// Pitch v2
		SynthDef("OSC DensityBand Pitch2",
			{arg busIn, seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
		...
		// DensityBand Audio Analyze KeyTrack
		SynthDef("OSC DensityBand KeyTrack",
		...
		// DensityBand Keyboard
		SynthDef("OSC DensityBand Keyboard",
			{arg busIn, note=60, amp=0.5, trigger=0;
				var input, timeIn=0;
				input = In.ar(busIn);
				timeIn = Timer.kr(trigger);
				SendReply.kr(trigger, '/DensityBand_Music_Data', values: [note, amp, timeIn], replyID: [1, 2, 3]);
		}).add;

		// DensityBand MIDI
		SynthDef("OSC DensityBand MIDI",
			{arg busIn, note=60, amp=0.5, trigger=0;
				var input, timeIn=0;
				input = In.ar(busIn);
				timeIn = Timer.kr(trigger);
		...
		SynthDef("FormantStreamEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				line = if(Rand(0, 1) < 0.5, XLine.kr(centroid, energy, durSynth), XLine.kr(energy, centroid, durSynth));
				// Play
				chain = Formant.ar(freq, line, 0.5) * envelope * Trig1.kr(Impulse.kr(flux * 100), flatness) * amp;
				// Out
				Out.ar(out, chain);
		}).add;
		...
		SynthDef("FMStreamEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				mod =SinOsc.ar(freq * modPartial, 0 , freq * index * LFNoise1.kr((centroid / energy).abs).abs);
				car = SinOsc.ar(freq * carPartial + mod);
				chain = car * 0.5 * envelope * Trig1.kr(Impulse.kr(flux * 100), flatness) * amp;
				// Out
				Out.ar(out, chain);
		}).add;
		...
		SynthDef("AnalogStringStreamEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				fc = LinExp.kr(LFNoise1.kr(flux), -1, 1, energy, centroid);
				osc = Mix.fill(8, {LFSaw.ar(freq * [Rand(0.99,1.01), Rand(0.99,1.01)], 0, 1) }).distort * 0.5;
				chain= RLPF.ar(osc, fc, flatness) * envelope * Trig1.kr(Impulse.kr(flux * 100), flatness) * amp;
				// Out
				Out.ar(out, chain);
		}).add;
		...
		SynthDef("PlayBufKlankStreamPreBuf",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), gate, 1, 0, durSynth.max(1), 2);
				// Play Buffer
				chain = PlayBuf.ar(1, buffer, BufRateScale.kr(buffer) * rate, gate, BufFrames.kr(buffer) * offset, 1);
		...
		SynthDef("PlayBufStreamPreBufEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				chain = HPplayBuf.ar(1, buffer, BufRateScale.kr(buffer) * rate, Trig1.kr(Impulse.kr(flux * 100), flatness), BufFrames.kr(buffer) * offset, 1, seuil: ctrlHP1, sensibilite: ctrlHP2) * envelope * amp;
				// Out
				Out.ar(out, chain);
		}).add;
		...
		SynthDef("PlayBufKlankStreamPreBufEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), gate, 1, 0, durSynth.max(1), 2);
				// Play Buffer
				chain = PlayBuf.ar(1, buffer, BufRateScale.kr(buffer) * rate, Trig1.kr(Impulse.kr(flux * 100), flatness), BufFrames.kr(buffer) * offset, 1);
				chain =  DynKlank.ar(`[[Rand(energy, centroid),Rand(energy, centroid),Rand(energy, centroid),Rand(energy, centroid),Rand(energy, centroid),Rand(energy, centroid)], 0.01, [0.16, 0.16, 0.16, 0.16, 0.16, 0.16]], chain, 1, 0, durSynth);
				chain = chain * envelope * amp;
				// Out
				Out.ar(out, chain);
		...
		SynthDef("BufRdRFPostBuf",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				flux=0.5, flatness=0.5, centroid=440, energy=440, bpm=1, ctrlHP1=0.5, ctrlHP2=0.5,
				envLevel1=0.0, envLevel2=1.0, envLevel3=1.0, envLevel4=0.75, envLevel5=0.75, envLevel6=0.5, envLevel7=0.5, envLevel8=0.0,  envTime1=0.015625, envTime2=0.109375, envTime3=0.25, envTime4=0.25, envTime5=0.125, envTime6=0.125, envTime7=0.125;
				var chain, inputSig, rate, envelope, recHead, line;
				// Normalize
				energy = (energy / 8372 * 4186).clip(50, 4186).lag(durSynth);
				centroid = (centroid / 12544 * 8372).clip(50, 8372).lag(durSynth);
				flatness = flatness.clip(0.001, 1.0).lag(durSynth);
				...
				recHead = Phasor.ar(0, BufRateScale.kr(buffer), 0, BufFrames.kr(buffer));
				BufWr.ar(inputSig, buffer, recHead, loop: 1);
				//RecordBuf.ar(inputSig, buffer, offset: 0, recLevel: level1, preLevel:level2, run: 1, loop: 1, trigger: 1);level2, run: 1, loop: 1, trigger: 1);
				// Set FHZ
				rate = 2**((freq.cpsmidi - 48).midicps).cpsoct * reverse;
				// Envelope
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), gate, amp, 0, dur, 2);
		...
		SynthDef("Warp1PostBuf",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), gate, amp, 0, dur, 2);
				// Play Buffer
				//chain = HPbufRd.ar(1, buffer, Phasor.ar(0, BufRateScale.kr(buffer) * rate, BufFrames.kr(buffer) * offset, recHead, BufFrames.kr(buffer) * offset), loop, seuil: ctrlHP1, sensibilite: ctrlHP2);
		...
		SynthDef("PV_AddStreamPrePostBuf",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), gate, 1, 0, durSynth.max(1), 2);
				// Play Buffer
				in1 = PlayBuf.ar(1, buffer, BufRateScale.kr(buffer) * rate, gate, BufFrames.kr(buffer) * offset, 1);
				in2 = HPbufRd.ar(1, postBuffer, Phasor.ar(0, BufRateScale.kr(postBuffer) * rate, BufFrames.kr(postBuffer) * offset, recHead, BufFrames.kr(postBuffer) * offset), 1, seuil: ctrlHP1, sensibilite: ctrlHP2);
				// FFT
				fft1 = FFT(LocalBuf(2048, 1), in1);
				fft2 = FFT(LocalBuf(2048, 1), in2);
		...
		SynthDef("PV_AddStreamPrePostBufEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), gate, 1, 0, durSynth.max(1), 2);
				// Play Buffer
				in1 = PlayBuf.ar(1, buffer, BufRateScale.kr(buffer) * rate, Trig1.kr(Impulse.kr(flux * 100), flatness), BufFrames.kr(buffer) * offset, 1);
				in2 = HPbufRd.ar(1, postBuffer, Phasor.ar(Trig1.kr(Impulse.kr(flux * 100), flatness), BufRateScale.kr(postBuffer) * rate, BufFrames.kr(postBuffer) * offset, recHead, BufFrames.kr(postBuffer) * offset), 1, seuil: ctrlHP1, sensibilite: ctrlHP2);
				// FFT
				fft1 = FFT(LocalBuf(2048, 1), in1);
				fft2 = FFT(LocalBuf(2048, 1), in2);
				chain = PV_Add(fft1, fft2);
		...
		SynthDef("SynthSamplerStreamEnv",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				chain = chain + (HPbufRd.ar(1, localBuf, Phasor.ar(Trig1.kr(Impulse.kr(flux * 100), flatness), BufRateScale.kr(localBuf) * rate, BufFrames.kr(localBuf) * offset, recHead, BufFrames.kr(localBuf) * offset), 1, seuil: ctrlHP1, sensibilite: ctrlHP2) * envR * amp);
				chain = chain;
				// Out
				Out.ar(out, Mix(chain));
		}).add;
		...
		SynthDef("GranulatorStreamPostBuf",
			{arg in=0, out=0, buffer, gate=1, loop=1, offset=0, reverse=1,
				...
				rate = 2**((freq.cpsmidi - 48).midicps).cpsoct * reverse;
				trig = Dust.kr(bpm);
				playHead = Phasor.ar(trig, rate * BufRateScale.kr(buffer), 0, 1, TRand.kr(0, 1, Trig1.kr(bpm)));
				// Envelope
				envelope = EnvGen.kr(Env.new([envLevel1,envLevel2,envLevel3,envLevel4,envLevel5,envLevel6,envLevel7,envLevel8],[envTime1,envTime2,envTime3,envTime4,envTime5,envTime6,envTime7], 'sine'), trig, 1, 0, dur, 2);
				// Synth
				//chain = BufRd.ar(1, buffer, playHead, loop: 0) * envelope;
