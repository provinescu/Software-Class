...
";
		...
		s.waitForBoot({
			...
			responder = OSCFunc.newMatching({arg msg, time, addr, recvPort, freq=0, amp=0, duree=0, centroid=0, flatness=0, energy=0, flux=0, bpm=0;
				...
				var instrumentName, soundName, fxName, file;
				wekOut = msg[1..];
			...
			oscMusicFFT = OSCFunc.newMatching({arg msg, time, addr, recvPort, freq=0, amp=0, timer=1, duree=0;
				...
				if(msg.at(2) == 1,
					...
					},
					{
						...
						// Data Music
						data = [rangeFreqintruments[0],//8
							rangeFreqintruments[1],
							rangeDBintruments[0].ampdb,
							rangeDBintruments[1].ampdb,
		...
		playInstruments = Tdef("Player", {
			...
			loop({arg time, indexBandFhz;
				...
				listeDataInstruments.do({arg data, index, tempo;
					var bpm;
					if(flagBPM == 'on', {
						if(flagMemory == 'on', {bpm = memoryDataBPM.at(0).postcs}, {bpm = dataBPM.at(0).postcs});
						if(bpm.size >=3, {
							tempo = bpm.mediane;
...
createGUI {
	...
	windowEar = Window("WekDensity" + typeMasterOut, Rect(0, 800, 715, 850), scroll: true);
	windowEar.alpha=1.0;
	windowEar.front;
	...
	EZKnob(windowEar, 80 @ 80, "BPM System", ControlSpec(7.5, 480, \exp, 1),
		{|ez| userBPM = ez.value / 60; TempoClock.default.tempo = userBPM}, 60, layout: \vert2).setColors(Color.grey(0.3), Color.magenta);
	// FX
...
initSynthDef {
	...
	SynthDef("OSC WekDensity Onsets",
		{arg busIn, seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
			...
			array2 = [freqIn.clip(20, 4186).cpsmidi, ampIn.ampdb, timeIn, bpm, centroid.clip(20, 12544).cpsmidi, energy.clip(20, 12544).cpsmidi,flux.clip(0.0001, 1), flatness.clip(0.0001, 1)];
			SendReply.kr(detect, '/WekDensity_MFCC_FFT', values: [array, array2], replyID: [1,2]);//13 MFCC + 8 FFT
	}).add;
	...
	SynthDef("OSC WekDensity Keyboard",
		{arg busIn, note=60, amp=0.5, trigger=0;
			...
			array2 = [note, amp.ampdb, timeIn, bpm, centroid.clip(20, 12544).cpsmidi, energy.clip(20, 12544).cpsmidi,flux.clip(0.0001, 1), flatness.clip(0.0001, 1)];
			SendReply.kr(trigger, '/WekDensity_MFCC_FFT', values: [array, array2], replyID: [1,2]);//13 MFCC + 8 FFT
	}).add;
