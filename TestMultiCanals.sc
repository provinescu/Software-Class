TestMultiCanals {
	...
	*new	{arg out=16, devIn="MutliTracks Recording", devOut="MutliTracks Recording";
	...
	panel {
		...
		// MasterFX
		w = Window.new("TestMultiCanals", Rect(100, 500, 100, 100), scroll: true);
	...
	initSynthDef {
		...
		SynthDef("Noise",
			{arg out = 0, pan = 0, amp = 0.25;
				...
				/*
				// Ambisonic
				ambisonic = PanB2.ar(main, pan);
				main = DecodeB2.ar(n, ambisonic[0], ambisonic[1], ambisonic[2]);
				*/


				// Multi Canals
				//main = PanAz.ar(n, main, circle, width: 1.0);
				main = PanAz.ar(n, main, pan, 1, width: 2, orientation: 0.5);// 3.5 pour front derriere moi
