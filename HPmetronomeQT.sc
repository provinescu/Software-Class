...
HPmetronomeQT {
	...
	*new	{
	...
	run {
		...
		patternFonction = {var octave, ratio, degre, difL, difH, pos=(scale.degrees.size - 1);
			patternFreqStart = [];
			patternFreqEnd = [];
			patternFreq = [];
			patternAmp = [];
			...
			// Amp
			beats.do({arg i; if(i == 0, {patternAmp = patternAmp.add(0.9)},{patternAmp = patternAmp.add(0.3)})});
			// Dur
