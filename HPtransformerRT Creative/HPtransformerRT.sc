/* HPtransformerRT V31.0.2 RELEASE - SuperCollider 3.14 class source.
...
non-destructive interference probe. V31.0.2 adds robust online conditioning,
multiresolution cached contexts and a decimated online stability guard.
The class name differs to coexist with HPtransformer.
*/
...
HPtransformerRT {
    ...
    normalizationState, multiResolutionState, stabilityGuardState,
    creativeState, performanceState;
    ...
    *new { |
    ...
    init { |
        ...
        this.resetCreativeRealtimeState;
        ^this;
    }
    ...
    // Signed velocity cleaner. Negative values are valid directions.
    cleanSizedVelocity { |velocity, size|
        var x;
        x = velocity;
        while({
            x.isArray and: { x.size == 1 and: { x[0].isArray } }
        }, {
            x = x[0];
        });
        ^Array.fill(size.asInteger.max(1), { |i|
    ...
    writeTrajectoryMemory { |input, target, surpriseValue|
        ^{
                    ...
                    acceleration =
                    if(lastLearnedVelocity.isArray, {
                        Array.fill(inputSize, { |i| velocity[i] - lastLearnedVelocity[i] });
                    }, {
    ...
    buildForward { |input, requestedWindowSize, requestedVelocity|
        ^{
                    ...
                    x =
                    this.applyMultiResolutionContext(
                        this.prepareRealtimeInput(input, false)
                    );
                    activeWindowSize =
                    if(
                    ...
                    // Ajouter les souvenirs rappel‚àö¬©s ‚àö‚Ä† l'attention.
    ...
    predict { |input, requestedWindowSize, requestedVelocity|
        if(unifiedCurrentRuntime.notNil and: { unifiedRuntimeRole.not }, {
            ...
            ^unifiedCurrentRuntime.predict(
                input, requestedWindowSize, requestedVelocity
            );
        });
        ^{
                    var state;
    ...
    learnEvent { |event|
        var creativeStart, creativeResult, noveltyScale, baseLearningRate;
        if(unifiedRuntimeRole.not and: { unifiedLearningEnabled.not }, { ^0.0 });
        if(this.shouldLearnCreative(event).not, { ^0.0 });
        creativeStart = Main.elapsedTime;
        baseLearningRate = learningRate;
        noveltyScale = 1.0 + (creativeState[\noveltyAdaptiveStrength]
            * creativeState[\lastNovelty].clip(0.0, 1.0));
        learningRate = (baseLearningRate * noveltyScale).clip(baseLearningRate, baseLearningRate * 1.5);
        creativeResult = {
                    var x;
                    var result;
                    var n;
                    ...
                    if(
                        totalEvents == 0,
                        {
                            ...
                            if(lastGeneratedVelocity.isArray.not, {
        ...
        performanceState[\lastLearnSeconds] = Main.elapsedTime - creativeStart;
        performanceState[\learnEMASeconds] = (0.92 * performanceState[\learnEMASeconds])
            + (0.08 * performanceState[\lastLearnSeconds]);
        performanceState[\learnMaxSeconds] = performanceState[\learnMaxSeconds]
            .max(performanceState[\lastLearnSeconds]);
        ^creativeResult;
    }
                        ...
                        },
                        {
                            ...
                            if(
                                magnitude < 0.000001,
                                {
                                    direction = Array.fill(outputSize, { |d|
                                        if(
                                            d < lastGeneratedVelocity.size,
                                            {
                                                lastGeneratedVelocity[d];
                                            },
                                            {
                                                0.0;
                                            }
    ...
    generateStep { |input|
        var creativeStart, creativeResult;
        creativeStart = Main.elapsedTime;
        if(unifiedCurrentRuntime.notNil and: { unifiedRuntimeRole.not }, {
            if(unifiedGenerationEnabled.not, { ^nil });
        ...
        creativeResult = {
                ...
                this.autoTuneStep;
                    this.applyCreativeOutput(cleanInput, result);
        }.value;
        performanceState[\lastGenerateSeconds] = Main.elapsedTime - creativeStart;
        performanceState[\generateEMASeconds] = (0.92 * performanceState[\generateEMASeconds])
            + (0.08 * performanceState[\lastGenerateSeconds]);
        performanceState[\generateMaxSeconds] = performanceState[\generateMaxSeconds]
            .max(performanceState[\lastGenerateSeconds]);
        ^creativeResult;
    }
    ...
    // ============================================================
    // V31.0.2 ADVANCED REAL-TIME CONDITIONING
    // Incremental statistics, decimated multiresolution summaries,
    // and a decimated stability snapshot. No history is rescanned.
    // ============================================================
    resetAdvancedRealtimeState {
        ...
        normalizationState[\mean] = Array.fill(inputSize, { 0.5 });
        normalizationState[\variance] = Array.fill(inputSize, { 0.0625 });
        ...
        multiResolutionState[\midCache] = Array.fill(inputSize, { 0.5 });
        multiResolutionState[\slowCache] = Array.fill(inputSize, { 0.5 });
    ...
    updateNormalizationState { |rawInput|
        ...
        inputSize.do({ |i|
            ...
            normalizationState[\variance][i] =
                (decay * normalizationState[\variance][i])
                + (oneMinus * delta * delta);
        });
    ...
    conditionInputVector { |input, update=false|
        ...
        active = normalizationState[\enabled]
            and: { normalizationState[\count] >= normalizationState[\warmup] };
        if(active.not, { ^raw });
        ...
        result = Array.fill(inputSize, { |i|
            z = (raw[i] - normalizationState[\mean][i])
                / (normalizationState[\variance][i] + epsilonValue).sqrt;
    ...
    updateMultiResolutionState { |rawInput|
        ...
        if((multiResolutionState[\counter] % multiResolutionState[\midDivider].asInteger.max(1)) == 0, {
            ...
            meanVector = Array.fill(inputSize, { |i| slice.collect({ |v| v[i] }).mean });
    ...
    prepareRealtimeInput { |input, update=false|
        // Classic Musical RT: direct cleaned input.
        ^this.rawCleanInputVector(input);
    }
    applyMultiResolutionContext { |input|
        ^this.rawCleanInputVector(input);
    }
    stabilityGuardAfter { |measuredLoss|
        ^measuredLoss;
    }
    ...
    // ============================================================
    // CREATIVE REAL-TIME CONTROLLER
    // Novelty-gated learning, residual autonomy and timing diagnostics.
    // ============================================================
    resetCreativeRealtimeState {
        creativeState = IdentityDictionary.new;
        creativeState[\noveltyLearningEnabled] = true;
        creativeState[\noveltyThreshold] = 0.018;
        creativeState[\noveltyMinInterval] = 1;
        creativeState[\noveltyMaxInterval] = 12;
        creativeState[\noveltyAdaptiveStrength] = 0.35;
        creativeState[\eventsSinceLearn] = 0;
        creativeState[\learnedByNovelty] = 0;
        creativeState[\skippedByNovelty] = 0;
        creativeState[\lastLearnedInput] = nil;
        creativeState[\lastNovelty] = 1.0;
        creativeState[\autonomy] = 0.35;
        creativeState[\residualMix] = 0.82;
        creativeState[\maxDelta] = 0.42;
        creativeState[\changeDensity] = 0.45;
        creativeState[\baseAttentionTemperature] = attentionTemperature;
        creativeState[\baseDeltaScale] = deltaScale;
        creativeState[\baseExplorationGain] = trajectoryExplorationGain;
        creativeState[\baseNoiseGain] = diversityNoiseGain;
        performanceState = IdentityDictionary.new;
        performanceState[\lastLearnSeconds] = 0.0;
        performanceState[\lastGenerateSeconds] = 0.0;
        performanceState[\learnEMASeconds] = 0.0;
        performanceState[\generateEMASeconds] = 0.0;
        performanceState[\learnMaxSeconds] = 0.0;
        performanceState[\generateMaxSeconds] = 0.0;
        ^this;
    }
    creativeNovelty { |input|
        var x, previous;
        x = this.rawCleanInputVector(input);
        previous = creativeState[\lastLearnedInput];
        if(previous.isNil, { ^1.0 });
        ^Array.fill(inputSize, { |i| (x[i] - previous[i]).abs }).mean.clip(0.0, 1.0);
    }
    shouldLearnCreative { |input|
        var novelty, since, minInterval, maxInterval, decision;
        if(creativeState.isNil, { this.resetCreativeRealtimeState });
        novelty = this.creativeNovelty(input);
        creativeState[\lastNovelty] = novelty;
        since = creativeState[\eventsSinceLearn] + 1;
        creativeState[\eventsSinceLearn] = since;
        if(creativeState[\noveltyLearningEnabled].not, { ^true });
        minInterval = creativeState[\noveltyMinInterval].asInteger.max(1);
        maxInterval = creativeState[\noveltyMaxInterval].asInteger.max(minInterval);
        decision = (since >= maxInterval) or: {
            (since >= minInterval) and: { novelty >= creativeState[\noveltyThreshold] }
        };
        if(decision, {
            creativeState[\eventsSinceLearn] = 0;
            creativeState[\lastLearnedInput] = this.rawCleanInputVector(input);
            creativeState[\learnedByNovelty] = creativeState[\learnedByNovelty] + 1;
        }, {
            creativeState[\skippedByNovelty] = creativeState[\skippedByNovelty] + 1;
        });
        ^decision;
    }
    setCreativeAutonomy { |amount|
        var a;
        if(creativeState.isNil, { this.resetCreativeRealtimeState });
        a = amount.asFloat.clip(0.0, 1.0);
        creativeState[\autonomy] = a;
        attentionTemperature = (0.78 + (0.62 * a)).clip(0.50, 1.60);
        deltaScale = (0.18 + (0.34 * a)).clip(0.10, 0.65);
        trajectoryExplorationGain = (0.0005 + (0.0095 * a)).clip(0.0, 0.02);
        diversityNoiseGain = (0.00002 + (0.00080 * a)).clip(0.0, 0.003);
        ^a;
    }
    applyCreativeOutput { |input, generated|
        var x, y, a, residualMix, density, maxDelta, acceptLarge;
        if(creativeState.isNil, { this.resetCreativeRealtimeState });
        x = this.rawCleanInputVector(input);
        y = this.cleanOutputVector(generated);
        a = creativeState[\autonomy].clip(0.0, 1.0);
        residualMix = creativeState[\residualMix].clip(0.0, 1.0);
        density = creativeState[\changeDensity].clip(0.0, 1.0);
        maxDelta = creativeState[\maxDelta].clip(0.01, 1.0);
        acceptLarge = density.coin;
        ^Array.fill(outputSize, { |i|
            var d, limited, residual, absolute;
            d = this.maskedDifference(x[i], y[i], i);
            limited = d.clip(maxDelta.neg, maxDelta);
            if(acceptLarge.not, { limited = limited * (0.35 + (0.65 * density)) });
            residual = this.maskedWrap(x[i] + (limited * a), i);
            absolute = ((y[i] * a) + (x[i] * (1.0 - a))).clip(0.0, 1.0);
            this.maskedWrap((residual * residualMix) + (absolute * (1.0 - residualMix)), i);
        });
    }
    creativeRealtimeStatus {
        ^(
            noveltyLearningEnabled: creativeState[\noveltyLearningEnabled],
            noveltyThreshold: creativeState[\noveltyThreshold],
            noveltyMinInterval: creativeState[\noveltyMinInterval],
            noveltyMaxInterval: creativeState[\noveltyMaxInterval],
            lastNovelty: creativeState[\lastNovelty],
            learnedByNovelty: creativeState[\learnedByNovelty],
            skippedByNovelty: creativeState[\skippedByNovelty],
            autonomy: creativeState[\autonomy],
            residualMix: creativeState[\residualMix],
            maxDelta: creativeState[\maxDelta],
            changeDensity: creativeState[\changeDensity],
            learnEMAms: performanceState[\learnEMASeconds] * 1000.0,
            generateEMAms: performanceState[\generateEMASeconds] * 1000.0,
            learnMaxMs: performanceState[\learnMaxSeconds] * 1000.0,
            generateMaxMs: performanceState[\generateMaxSeconds] * 1000.0
        );
    }
    setCreativeLiveConcert {
        this.setParameters((learningRate:0.00030,gradientClip:0.45,replayRate:0.03,
            replayBatchSize:1,protectionStrength:0.18,memoryRetrievalGain:0.08,
            memoryRecallSize:1,trajectoryRetrievalGain:0.05,trajectoryRecallSize:1,
            generationWindowSize:4,autoTuneEnabled:false,metaLearnEnabled:false,
            noveltyLearningEnabled:true,noveltyThreshold:0.018,noveltyMinInterval:1,
            noveltyMaxInterval:12,autonomy:0.35,residualMix:0.82,maxDelta:0.42,
            changeDensity:0.45),false);
        this.disableUnifiedRCU;this.enableLearning;this.enableGeneration;
        ^this.runtimeConfig;
    }
    ...
    // ============================================================
    // RUNTIME CONTROL API
    // Les param‚àö¬Ætres ci-dessous peuvent ‚àö‚Ñ¢tre modifi‚àö¬©s entre deux
    // appels ‚àö‚Ä† generateStep, y compris depuis une Routine, un GUI,
    ...
    runtimeParameterNames {
        ^[
            ...
            \noveltyLearningEnabled, \noveltyThreshold, \noveltyMinInterval,
            \noveltyMaxInterval, \noveltyAdaptiveStrength,
            \autonomy, \residualMix, \maxDelta, \changeDensity,
            ...
            \normalizationEnabled, \normalizationDecay, \normalizationClip,
            \normalizationBlend, \normalizationWarmup,
            \multiResolutionEnabled, \multiResolutionMidWindow, \multiResolutionSlowWindow,
            \multiResolutionMidDivider, \multiResolutionSlowDivider,
            \multiResolutionMidGain, \multiResolutionSlowGain,
            \stabilityGuardEnabled, \stabilitySnapshotInterval,
            \stabilityLossLimit, \stabilityLearningRateBackoff,
            \torusMask
        ];
    }
    ...
    getParameter { |name|
        ...
        ^switch(key,
            ...
            \noveltyLearningEnabled, { creativeState[\noveltyLearningEnabled] },
            \noveltyThreshold, { creativeState[\noveltyThreshold] },
            \noveltyMinInterval, { creativeState[\noveltyMinInterval] },
            \noveltyMaxInterval, { creativeState[\noveltyMaxInterval] },
            \noveltyAdaptiveStrength, { creativeState[\noveltyAdaptiveStrength] },
            \autonomy, { creativeState[\autonomy] },
            \residualMix, { creativeState[\residualMix] },
            \maxDelta, { creativeState[\maxDelta] },
            \changeDensity, { creativeState[\changeDensity] },
            ...
            \normalizationWarmup, { normalizationState[\warmup] },
            \multiResolutionEnabled, { multiResolutionState[\enabled] },
            \multiResolutionMidWindow, { multiResolutionState[\midWindow] },
            \multiResolutionSlowWindow, { multiResolutionState[\slowWindow] },
            \multiResolutionMidDivider, { multiResolutionState[\midDivider] },
            \multiResolutionSlowDivider, { multiResolutionState[\slowDivider] },
            \multiResolutionMidGain, { multiResolutionState[\midGain] },
            \multiResolutionSlowGain, { multiResolutionState[\slowGain] },
            \stabilityGuardEnabled, { stabilityGuardState[\enabled] },
            \stabilitySnapshotInterval, { stabilityGuardState[\snapshotInterval] },
            \stabilityLossLimit, { stabilityGuardState[\lossLimit] },
            \stabilityLearningRateBackoff, { stabilityGuardState[\learningRateBackoff] },
    ...
    setParameterLocal { |name, value, post=true|
        ...
        // Les tailles de matrices et de m‚àö¬©moires allou‚àö¬©es ne doivent
        // pas changer ‚àö‚Ä† chaud. Une nouvelle instance est n‚àö¬©cessaire.
        if([
        ...
        switch(key,
            ...
            \noveltyLearningEnabled, {
                creativeState[\noveltyLearningEnabled] = (value == true)
                    or: { value.isNumber and: { value > 0 } };
            },
            \noveltyThreshold, { creativeState[\noveltyThreshold] = value.asFloat.clip(0.0, 1.0) },
            \noveltyMinInterval, { creativeState[\noveltyMinInterval] = value.asInteger.clip(1, 1024) },
            \noveltyMaxInterval, {
                creativeState[\noveltyMaxInterval] = value.asInteger.clip(
                    creativeState[\noveltyMinInterval], 4096
                );
            },
            \noveltyAdaptiveStrength, {
                creativeState[\noveltyAdaptiveStrength] = value.asFloat.clip(0.0, 1.0);
            },
            \autonomy, { this.setCreativeAutonomy(value) },
            \residualMix, { creativeState[\residualMix] = value.asFloat.clip(0.0, 1.0) },
            \maxDelta, { creativeState[\maxDelta] = value.asFloat.clip(0.01, 1.0) },
            \changeDensity, { creativeState[\changeDensity] = value.asFloat.clip(0.0, 1.0) },
        ...
        // Les trois poids de perte sont normalis‚àö¬©s pour conserver
        // une ‚àö¬©chelle de perte stable lors d'un contr‚àö¬•le en direct.
        if([
    ...
    // Morphing asynchrone d'un param‚àö¬Ætre. ‚àö√Ñ lancer depuis AppClock
    ...
    resetLearning {
        ^{
                ...
                metaLearnRetrievalGainMax = 0.30;
                    "V31.0.2 Learning+ Dynamic Trajectory Memory runtime and optimizer reset / weights and memories preserved".postln;
                    nil;
    ...
    resetMemory {
        ^{
                    ...
                    Array.new;
                    protectionScalar = 0.0;
                    "V31.0.2 Learning+ Dynamic Trajectory Memory memory reset".postln;
                    nil;
    ...
    resetAll {
        ^{
                    ...
                    // ------------------------------------------------------------
                    this.resetLearning;
                    "V31.0.2 Learning+ Dynamic Trajectory Memory full reset".postln;
                    nil;
    ...
    statusSilent {
        ...
        ^(
            ...
            stabilityRollbacks: stabilityGuardState[\rollbacks],
            stabilityLastReason: stabilityGuardState[\lastReason],
            creativeRealtime: this.creativeRealtimeStatus,
            learnedEvents: learnedEvents,
            adamStep: adamStep,
            totalEvents: totalEvents,
            prediction: lastPrediction.copy,
            delta: lastDelta.copy,
            learnedVelocity: lastLearnedVelocity.copy,
            generatedVelocity: lastGeneratedVelocity.copy
        );
    }
    status {
        ^{
                    ...
                    "".postln;
                    "============================================================".postln;
                    "TRANSFORMER VECTOR V31.0.2 Learning+ Dynamic Trajectory Memory".postln;
                ...
                ("metaLearnStabilityPressure: " ++ metaLearnStabilityPressure).postln;
                    ...
                    "delta:".postln;
                    lastDelta.postln;
                    "learnedVelocity:".postln;
                    lastLearnedVelocity.postln;
                    "generatedVelocity:".postln;
                    lastGeneratedVelocity.postln;
                    "============================================================".postln;
                    (
                        version: 31.01,
                        parameters: parameterCount,
    ...
    saveArchive { |path|
        ...
        ("HPtransformerRT V31.0.2 saved: " ++ finalPath).postln;
        ^finalPath;
    }
    *readArchive { |path|
        var finalPath, model;
        ...
        ("HPtransformerRT V31.0.2 restored: " ++ finalPath).postln;
        ^model;
    }
    ...
    exportGenerationState {
        ^(
            ...
            lastGeneratedVelocity: this.copySnapshotValue(lastGeneratedVelocity),
    ...
    loadGenerationState { |state|
        ...
        lastGeneratedVelocity = if(state[\lastGeneratedVelocity].notNil, {
            state[\lastGeneratedVelocity];
        }, {
            if(state[\lastObservedVelocity].notNil, {
                state[\lastObservedVelocity];
            }, {
                this.zeroVector(inputSize);
            });
        });
