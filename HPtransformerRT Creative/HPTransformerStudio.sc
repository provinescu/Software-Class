/*
HPTransformerStudio V9.0.0 OSC temps reel et profils d usage for HPtransformerRT V31.0.0 - SuperCollider 3.14
...
HPTransformerStudio : Object {
    ...
    *new { |transformer, externalTransformerKey=\hpTR|
    ...
    build {
        // HPTransformer Studio Pro V9.0.0 PROFILS TEMPS REEL / HORS TEMPS REEL - HPtransformerRT V31.0.0 - SuperCollider 3.14
        ...
        var cpuGovernorEnabled=true, cpuLevel=0, cpuBudgetMs=12.0, cpuLastChange=0.0;
        var cpuStatusText, cpuGovernorButton, updateCpuGovernor;
        ...
        generalNames=["Equilibre","Apprentissage rapide","Stable","Exploration","Memoire forte",
         ...
         "Classic Musical Direct","Classic Live Concert","Creative RT - Live Concert","Classic Generation Only",
        ...
        generalPresets=[
         ...
         // Creative RT Live Concert: nouveaute, autonomie et gouverneur CPU.
         (learningRate:0.00030,gradientClip:0.45,replayRate:0.03,replayBatchSize:1,protectionStrength:0.18,memoryRetrievalGain:0.08,memoryRecallSize:1,trajectoryRetrievalGain:0.05,trajectoryRecallSize:1,generationWindowSize:4,noveltyLearningEnabled:true,noveltyThreshold:0.018,noveltyMinInterval:1,noveltyMaxInterval:12,noveltyAdaptiveStrength:0.35,autonomy:0.35,residualMix:0.82,maxDelta:0.42,changeDensity:0.45,autoTuneEnabled:false,metaLearnEnabled:false),
        ...
        generalProcessHz=Array.fill(15,{nil})++[20.0,60.0,50.0,40.0,30.0,10.0,20.0,15.0,20.0,15.0,25.0,20.0,30.0,40.0,25.0,15.0,20.0,20.0,20.0,20.0,20.0];
        generalLearnDivider=Array.fill(15,{nil})++[8,4,3,4,8,1,16,8,8,8,8,8,4,3,4,8,8,6,6,16,4];
        generalLearningState=Array.fill(15,{nil})++[true,true,true,true,false,true,false,true,true,true,false,true,true,true,true,true,true,true,true,false,true];
        generalGenerationState=Array.fill(15,{nil})++[true,true,true,true,true,false,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true];
        ...
        w=Window("HPTransformer Studio Pro V9.0.0 - HPtransformerRT V31.0.0 - Profils temps reel et hors temps reel",uiRect.(35,35,1320,860)).background_(Color.grey(0.13));
        ...
        StaticText(pages[\dashboard],uiRect.(820,675,450,80)).string_(
         "Profils generaux V9.0.0 :"++Char.nl++
         ...
         "Creative RT : nouveaute, autonomie 0.35, CPU auto."++Char.nl++
        ...
        cpuGovernorButton=Button(pages[\osc],uiRect.(1125,216,135,30)).states_([
         ["CPU AUTO OFF",Color.white,Color.red(0.52)],["CPU AUTO ON",Color.white,Color.green(0.42)]])
         .value_(1).action_({|b|cpuGovernorEnabled=(b.value==1);if(cpuGovernorEnabled.not,{cpuLevel=0})});
        cpuStatusText=TextView(pages[\osc],uiRect.(20,655,1240,55)).editable_(false)
         .background_(Color(0.96,0.97,0.98)).stringColor_(Color.black).font_(Font("Monaco",9));
        ...
        updateCpuGovernor={
         var perf,loadMs,now,newLevel;
         if(cpuGovernorEnabled.not,{^nil});
         perf={transformer.creativeRealtimeStatus}.try ? ();
         loadMs=(perf[\learnEMAms]?0.0)+(perf[\generateEMAms]?0.0);
         now=Main.elapsedTime;
         newLevel=if(loadMs>(cpuBudgetMs*1.65),{3},{if(loadMs>(cpuBudgetMs*1.25),{2},{if(loadMs>cpuBudgetMs,{1},{0})})});
         if((newLevel!=cpuLevel) and:{(now-cpuLastChange)>2.0},{
          cpuLevel=newLevel;cpuLastChange=now;
          if(cpuLevel==0,{oscLearningDivider=6;transformer.setParameter(\replayRate,0.03,false);transformer.setParameter(\generationWindowSize,4,false)});
          if(cpuLevel==1,{oscLearningDivider=8;transformer.setParameter(\replayRate,0.0,false)});
          if(cpuLevel==2,{oscProcessHz=15;oscLearningDivider=12;transformer.setParameter(\memoryRecallSize,0,false);transformer.setParameter(\trajectoryRecallSize,0,false);transformer.setParameter(\generationWindowSize,2,false)});
          if(cpuLevel==3,{oscProcessHz=12;oscLearningDivider=16;transformer.disableLearning});
          if(oscProcessHzBox.notNil,{oscProcessHzBox.value_(oscProcessHz)});
          if(oscLearningDividerBox.notNil,{oscLearningDividerBox.value_(oscLearningDivider)});
          addLog.("Gouverneur CPU niveau "++cpuLevel++" / charge "++loadMs.round(0.01)++" ms")
         });
         if(cpuStatusText.notNil,{cpuStatusText.string_((enabled:cpuGovernorEnabled,level:cpuLevel,budgetMs:cpuBudgetMs,loadEMAms:loadMs,performance:perf).asCompileString)});
        };
        ...
        [statusText,outputView,generationLiveText,memoryLiveText,surpriseLiveText,oscStatusText,oscEventsView,genPresetText,memoryPresetText,surprisePresetText,autoText,metaText,rcuText,logView,torusMaskStatus,setDescriptionView,manualParamsStatus,morphStatus,cpuStatusText].do({|view|if(view.notNil,{view.background_(Color(0.96,0.97,0.98));view.stringColor_(Color.black);view.font_(Font.default.size_(12));view.refresh})});
        ...
        w.onClose_({running=false;oscStop.value;if(routine.notNil,{routine.stop});window=nil;refreshRoutine=nil});refreshStateIndicators.value;showPage.(\dashboard);addLog.("Studio Pro V9.0.0 pour HPtransformerRT V31.0.0 ouvert - echelle "++uiScale);window=w;refreshRoutine=routine;scrollView.visibleOrigin_(Point(0,0));w.front;
    }
}
