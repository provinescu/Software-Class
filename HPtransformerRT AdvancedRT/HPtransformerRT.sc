/* HPtransformerRT V30.2.3 RELEASE - SuperCollider 3.14 class source.
...
non-destructive interference probe. V30.2.3 adds robust online conditioning,
optional multiresolution cached contexts and a decimated online stability guard.
V30.2.3 Intel Musical RT keeps the 8 x 8 structure but disables Advanced RT
processing by default to preserve immediacy, variation and CPU headroom.
...
HPtransformerRT {
    ...
    *new { |
    ...
    // ============================================================
    // V30.2.3 ADVANCED REAL-TIME CONDITIONING
    ...
    resetAdvancedRealtimeState {
        ...
        normalizationState[\blend] = 0.03;
        normalizationState[\warmup] = 64;
        ...
        multiResolutionState[\midDivider] = 8;
        multiResolutionState[\slowDivider] = 32;
        multiResolutionState[\midGain] = 0.0;
        multiResolutionState[\slowGain] = 0.0;
        ...
        stabilityGuardState[\snapshotInterval] = 256;
    ...
    prepareRealtimeInput { |input, update=false|
        var raw, conditioned;
        raw = this.rawCleanInputVector(input);
        // Fast bypass for Intel real-time use.
        if(update and: { normalizationState[\enabled] }, {
            this.updateNormalizationState(raw);
        });
        if(update and: { multiResolutionState[\enabled] }, {
            this.updateMultiResolutionState(raw);
        });
        conditioned = if(normalizationState[\enabled], {
            this.conditionInputVector(raw, false);
        }, { raw });
        ^conditioned;
    }
    applyMultiResolutionContext { |input|
        var x, midGain, slowGain, totalGain;
        x = this.rawCleanInputVector(input);
        if(multiResolutionState.isNil or: { multiResolutionState[\enabled].not }, { ^x });
        midGain = multiResolutionState[\midGain].clip(0.0, 0.45);
        slowGain = multiResolutionState[\slowGain].clip(0.0, 0.45);
        totalGain = (midGain + slowGain).min(0.90);
        ^Array.fill(inputSize, { |i|
            ((x[i] * (1.0 - totalGain))
                + (multiResolutionState[\midCache][i] * midGain)
                + (multiResolutionState[\slowCache][i] * slowGain)).clip(0.0, 1.0);
        });
    }
    stabilityGuardAfter { |measuredLoss|
        var invalid, reason, snapshot;
        if(stabilityGuardState.isNil, { this.resetAdvancedRealtimeState });
        if(stabilityGuardState[\enabled].not, { ^measuredLoss });
        stabilityGuardState[\counter] = stabilityGuardState[\counter] + 1;
        invalid = measuredLoss.isNumber.not
            or: { measuredLoss.isNaN }
            or: { measuredLoss.abs > stabilityGuardState[\lossLimit] };
        if(invalid, {
            reason = if(measuredLoss.isNumber.not or: { measuredLoss.isNaN }, { \nonFiniteLoss }, { \lossLimit });
            snapshot = stabilityGuardState[\lastStableSnapshot];
            if(snapshot.notNil, {
                this.loadInferenceSnapshot(snapshot);
                this.resetOptimizerState;
            });
            learningRate = (learningRate * stabilityGuardState[\learningRateBackoff]).max(0.0000001);
            learningRateCurrent = learningRate;
            stabilityGuardState[\rollbacks] = stabilityGuardState[\rollbacks] + 1;
            stabilityGuardState[\lastReason] = reason;
        }, {
            if((stabilityGuardState[\counter] % stabilityGuardState[\snapshotInterval].asInteger.max(1)) == 0, {
                stabilityGuardState[\lastStableSnapshot] = this.exportInferenceSnapshot;
                stabilityGuardState[\lastReason] = \stableSnapshot;
            });
        });
        ^measuredLoss;
    }
    ...
    // ============================================================
    // INTEL MUSICAL REAL-TIME PROFILES
    // Apply directly to learnEvent, predict, generateStep and generate.
    // ============================================================
    setIntelMusicalDirect {
        this.setParameters((
            normalizationEnabled: false, multiResolutionEnabled: false,
            stabilityGuardEnabled: false, learningRate: 0.00020,
            gradientClip: 0.40, replayRate: 0.04, replayBatchSize: 1,
            protectionStrength: 0.14, memoryRetrievalGain: 0.07,
            memoryRecallSize: 1, trajectoryRetrievalGain: 0.05,
            trajectoryRecallSize: 1, attentionTemperature: 1.10,
            routerTemperature: 1.00, deltaScale: 0.32,
            trajectoryExplorationGain: 0.004, diversityNoiseGain: 0.00025,
            diversityRepulsionGain: 0.0012, autoTuneEnabled: false,
            metaLearnEnabled: false
        ), false);
        this.disableUnifiedRCU;
        ^this.runtimeConfig;
    }
    setIntelUltraCPUFastLearn {
        // Minimum CPU with fast adaptation: no replay, no retrieval, no RCU,
        // no Advanced RT, short generation context and one external learn in four.
        // The Learn/N decimation is managed by the Studio preset.
        this.setParameters((
            normalizationEnabled: false, multiResolutionEnabled: false,
            stabilityGuardEnabled: false, learningRate: 0.00065,
            gradientClip: 0.70, surpriseThreshold: 0.030,
            surpriseGain: 1.80, adaptationFastRate: 2.20,
            adaptationSlowRate: 0.55, replayRate: 0.0,
            replayBatchSize: 1, protectionStrength: 0.08,
            memoryRetrievalGain: 0.0, memoryRecallSize: 0,
            trajectoryRetrievalGain: 0.0, trajectoryRecallSize: 0,
            generationWindowSize: 2, attentionTemperature: 1.15,
            routerTemperature: 1.00, deltaScale: 0.38,
            trajectoryExplorationGain: 0.005,
            diversityNoiseGain: 0.00030,
            diversityRepulsionGain: 0.0012,
    ...
    setIntelGenerationOnly {
        this.setParameters((
            normalizationEnabled: false, multiResolutionEnabled: false,
            stabilityGuardEnabled: false, replayRate: 0.0,
            memoryRecallSize: 1, trajectoryRecallSize: 1,
            attentionTemperature: 1.20, routerTemperature: 1.00,
            deltaScale: 0.36, trajectoryExplorationGain: 0.006,
            diversityNoiseGain: 0.00040, diversityRepulsionGain: 0.0018,
            autoTuneEnabled: false, metaLearnEnabled: false
        ), false);
        this.disableLearning; this.enableGeneration; this.disableUnifiedRCU;
        ^this.runtimeConfig;
    }
    setAdvancedRealtimeOff {
        this.setParameters((normalizationEnabled:false,
            multiResolutionEnabled:false, stabilityGuardEnabled:false), false);
        ^this.runtimeConfig;
    }
    // ============================================================
    // RUNTIME CONTROL API
    ...
    resetLearning {
        ^{
                ...
                metaLearnRetrievalGainMax = 0.30;
                    "V30.2.3 Learning+ Dynamic Trajectory Memory runtime and optimizer reset / weights and memories preserved".postln;
                    nil;
    ...
    resetMemory {
        ^{
                    ...
                    Array.new;
                    protectionScalar = 0.0;
                    "V30.2.3 Learning+ Dynamic Trajectory Memory memory reset".postln;
                    nil;
    ...
    resetAll {
        ^{
                    ...
                    // ------------------------------------------------------------
                    this.resetLearning;
                    "V30.2.3 Learning+ Dynamic Trajectory Memory full reset".postln;
                    nil;
    ...
    status {
        ^{
                    ...
                    "".postln;
                    "============================================================".postln;
                    "TRANSFORMER VECTOR V30.2.3 Learning+ Dynamic Trajectory Memory".postln;
    ...
    saveArchive { |path|
        ...
        ("HPtransformerRT V30.2.3 saved: " ++ finalPath).postln;
        ^finalPath;
    }
    *readArchive { |path|
        var finalPath, model;
        ...
        ("HPtransformerRT V30.2.3 restored: " ++ finalPath).postln;
        ^model;
    }
