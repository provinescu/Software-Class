...
LiveCode2 {
	...
	var lastTime, oscMusic,  windowGVerb, tuning, degrees, root, scale, flagScaling, typeMasterOut, rangeDBintruments, rangeFreqintruments, quantizationDuree, stretchDuree, rangeDureeintruments, freqFiltreGUI, ampFiltreGUI, durFiltreGUI, dureeMaximumAnalyze, fhzFilter, ampFilter, dureeFilter, flagAlgoAnalyze, plotDataFFT, plotDataMusic, userBPM, setupKeyboardShortCut, fonctionShortCut, keyboardShortCut, shortCutCommande, fonctionShortCutCommande, listeFileAnalyze, listeMasterOut, listeNameFileAnalyze, formatRecordingMenu, recChannels, midiMenu, helpLiveCode, flagMidiOut, masterAppAddr, slaveAppAddr, oscStateFlag, ardourOSC, indexWindows, pathData, oscMenu, globalLiveCode, fonctionLoadPreset, fonctionSavePreset, fonctionCollectFolders, foldersToScanAll, foldersToScanPreset, stringFormat, busSynthInOut, listeBuffer, fonctionLoadSoundOrchestra, playInstruments, windowGlobal, pathSound, soundOrchestra, soundMenu, fxMenu, synthMenu, fxOrchestra, synthOrchestra, listeBusOff, maximumInstruments;
	...
	var rangeFlux, rangeFlatness, rangeCentroid, rangeEnergy;

	*new {arg path = "~/Documents/LiveCode/", ni = 26, numberOut=2, numberRec=2, format=0, devIn="Built-in Microph", devOut="Built-in Output", size = 256, wid=2.0, ori=0.5, flag=0, name="LiveCode", wek=6448, wekPort=57120, scPort=57110;
		...
		fonctionRecOn={
			...
			if(flagRecording == 'off', {
				...
				s.bind{
					s.recChannels_(recChannels);
					s.sync;
					s.prepareForRecord("~/Music/SuperCollider Recordings/".standardizePath ++ "LiveCode_" ++ Date.localtime.stamp ++ ".aiff");
					s.sync;
					s.record;
					s.sync;
				};
	...
	run {
		...
		"Please Wait... Loading LiveCode...".postln;
							...
							if(cmd == 'preset',
								{
									number = msg[item+2].asInteger.postln;
									{
										if(File.exists(pathData ++ "Preset" + number.value.asString ++ ".scd"), {
											...
											windowEar.name = "LiveCode" + " | " + "Preset" + number.asString;
		...
		s.waitForBoot({
			...
			"Please Wait... Loading LiveCode... Sending SynthDef...".postln;
			...
			synthAnalyseFFT = Synth.newPaused("OSC LiveCode FFT",
				['busIn', busAnalyzeIn, \speed, 24], groupeAnalyse, \addToTail); // 24 * each second
			s.sync;

			// Synth audio analyze Onsets
			synthAnalyseOnsets = Synth.newPaused("OSC LiveCode Onsets",
				['busIn', busAnalyzeIn], groupeAnalyse, \addToTail);
			s.sync;

			// Synth audio analyze Pitch
			synthAnalysePitch = Synth.newPaused("OSC LiveCode Pitch",
				['busIn', busAnalyzeIn], groupeAnalyse, \addToTail);
			s.sync;

			// Synth audio analyze Pitch Version 2
			synthAnalysePitch2 = Synth.newPaused("OSC LiveCode Pitch2",
				['busIn', busAnalyzeIn], groupeAnalyse, \addToTail);
			s.sync;

			// Synth audio analyze KeyTrack
			synthAnalyseKeyTrack = Synth.newPaused("OSC LiveCode KeyTrack",
				['busIn', busAnalyzeIn], groupeAnalyse, \addToTail);
			s.sync;
			...
			windowEar.name = "LiveCode" + typeMasterOut;
			...
			}, '/LiveCode_FFT_Data', serveurAdresse);
			...
			buildSynth = {arg indexBandFhz;
				...
				// DureeStretchBPM
				dureeStretchBPM =  newDur.sum * globalLiveCode.reciprocal;
				// Buffer
			...
			playInstruments = Tdef("Player", {
				...
				loop({arg time, indexBandFhz;
					...
					if(maximumInstruments > listeDataInstruments.size and: {(time - lastTime) >= (1 - globalLiveCode)}, {
						// With Band
						if(flagFhzBand == 'on' and: {rangeSynthBand.size != 0}, {
							...
							if(flagMemory == 'on', {
								if(memoryDataFreq.at(indexBandFhz) != [] and: {rrand(0.0, 1.0) < globalLiveCode},
									{
										buildSynth.value(indexBandFhz)});
							},
							{
								if(dataFreq.at(indexBandFhz) != [] and: {rrand(0.0, 1.0) < globalLiveCode},
									{
										buildSynth.value(indexBandFhz)});
							});
						},
...
";
		...
		MainMenu.register(menuFile.title_("File for Analyze"), "LiveCodeTools");
		...
		menuPreset = Menu(
			MenuAction("Load Preset", {Dialog.openPanel({ arg path, file;
				...
				fonctionLoadPreset.value(file.readAllString.interpret);
				file.close;
				windowEar.name="LiveCode" + typeMasterOut + " | " + PathName.new(path).fileName;
			},
			{"cancelled".postln})}),
			MenuAction("Save Preset",{Dialog.savePanel({arg path, name, pathonly, fileName, file;
				...
				file.close;
				windowEar.name = "LiveCode" + typeMasterOut + " | " + fileName;
			}, {"cancelled".postln})});
		);
		MainMenu.register(menuPreset.title_("Preset"), "LiveCodeTools");
		...
		MainMenu.register(menuInitAll.title_("Synth/Sound/FX"), "LiveCodeTools");
		...
		MainMenu.register(menuRecording.title_("Recording"), "LiveCodeTools");
		...
		MainMenu.register(menuAudio.title_("Audio"), "LiveCodeTools");
		...
		MainMenu.register(menuMIDI.title_("Midi"), "LiveCodeTools");
		...
		menuOSC = Menu(
			...
			MenuAction("OSC Master", {oscStateFlag='master';
				"LiveCode is OSC now !".postln;
			}),
			MenuAction("OSC Slave", {oscStateFlag='slave';
				"LiveCode is OSC SLAVE now !".postln;
			}),
			MenuAction("OSC Off", {oscStateFlag='off';
				"OSC is OFF now !".postln;
			});
		);
		MainMenu.register(menuOSC.title_("OSC"), "LiveCodeTools");

		menuAlgo = Menu(
			MenuAction("Not Activate", {nil}),
		);
		MainMenu.register(menuAlgo.title_("Algorithm"), "LiveCodeTools");

		menuHelp = MenuAction("Help ShortCut", {
			//Document.new("ShortCut for LiveCode", helpLiveCode);
			TextView().name_("ShortCut for LiveCode").string_(helpLiveCode).front;
		});
		MainMenu.register(menuHelp, "LiveCodeTools");
			...
			window.view.keyDownAction = {arg view,char,modifiers,unicode, keycode, number, file;
				...
				if(modifiers == 524288 and: {unicode==105} and: {keycode==34},{
					...
					if(File.exists(pathData ++ "Init Preset.scd"), {
						windowEar.name = "LiveCode" + " | " + "Init Preset";
				...
				if(char == $z, {number = rrand(0, foldersToScanPreset.size - 1);
					if(File.exists(pathData ++ foldersToScanPreset.at(number)),
						{file=File(pathData ++ foldersToScanPreset.at(number),"r");
							windowEar.name = "LiveCode" + typeMasterOut + " | " + foldersToScanPreset.at(number);
							fonctionLoadPreset.value(file.readAllString.interpret);
				...
				if(char == $k, {
					FileDialog.new({arg path;
						...
						windowEar.name="LiveCode" + " | " + pathData.asString;
						fonctionCollectFolders.value;
					}, fileMode: 2);
				});
				//key a // init genetic
		...
		////// Window Plotter Data /////
		windowPlotterData = Window("Freq | Amp | Duree | BPM | Centroid | Energy | Flux | Flatness", Rect(710, 800, 510, 510), scroll: true);
		...
		/////// LiveCode Ear Panel
		windowEar = Window("LiveCode" + typeMasterOut, Rect(0, 800, 715, 850), scroll: true);
		windowEar.alpha=1.0;
		windowEar.front;
	...
	initSynthDef {
		...
		// LiveCode Audio Analyze FFT
		SynthDef("OSC LiveCode FFT",
			{arg busIn, speed=24;
				...
				SendReply.kr(Impulse.kr(speed), '/LiveCode_FFT_Data', values: [flux.clip(0.0001, 1), flatness.clip(0.0001, 1), centroid.clip(20, 12544), energy.clip(20, 12544), bpm], replyID: [1, 2, 3, 4, 5]);
		}).add;
		...
		SynthDef("OSC LiveCode Onsets",
			{arg busIn, seuil=0.5, filtre=0.5, hzPass=440, ampInput = 1, ampLoPass = 0,  ampHiPass = 0;
				...
				SendReply.kr(detect, '/LiveCode_Music_Data', values: [freqIn, ampIn, timeIn], replyID: [1, 2, 3]);
		}).add;
		...
		SynthDef("OSC LiveCode Keyboard",
			{arg busIn, note=60, amp=0.5, trigger=0;
				...
				SendReply.kr(trigger, '/LiveCode_Music_Data', values: [note, amp, timeIn], replyID: [1, 2, 3]);
		}).add;
